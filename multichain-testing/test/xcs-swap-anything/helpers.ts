...
export const osmosisSwapTools = async t => {
  ...
  const createPoolAgainstOsmo = async (
    ...
    const hash = await getDenomHash('osmosis', originChain, originDenom);
  ...
  const isRouteSet = async (originChain, originDenom) => {
    try {
      await getPoolRoute(originChain, originDenom);
      return true;
    } catch (error) {
      console.error(
        `Pool Route is not set for ${originChain} asset ${originDenom}`,
        error,
      );
      return false;
    }
  ...
  const getPoolRoute = async (originChain, originDenom) => {
    const swaprouterAddress = xcsContracts.swaprouter.address;
    const hash = await getDenomHash('osmosis', originChain, originDenom);
    const queryMsg = {
      get_route: {
