...
alias cosmos-exec="kubectl exec -i cosmoshublocal-genesis-0 -c validator -- gaiad"
alias hermes-exec="kubectl exec -i hermes-osmosis-cosmoshub-0 -c relayer -- hermes"

COSMOS_WALLET="test1"
COSMOS_ADDRESS=$(cosmos-exec keys show ${COSMOS_WALLET} -a)
...
COSMOS_OSMOSIS_CHANNEL=$(echo "$CHANNEL_INFO" | jq -r '.channel_a')
OSMOSIS_COSMOS_CHANNEL=$(echo "$CHANNEL_INFO" | jq -r '.channel_b')
COSMOS_OSMOSIS_PORT="transfer"

COSMOS_TOKEN_DENOM="$1"
COSMOS_TOKEN_TRANSFER_AMOUNT="250000000000"

IBC_DENOM=$(echo -n "$COSMOS_OSMOSIS_PORT/$OSMOSIS_COSMOS_CHANNEL/$COSMOS_TOKEN_DENOM" | shasum -a 256 | awk '{print toupper($1)}')
...
POOL_CONFIG_FILE="$COSMOS_TOKEN_DENOM-$TOKEN_OUT_DENOM-pool-config.json"
...
cosmos_balance_json=$(cosmos-exec query bank balances $COSMOS_ADDRESS --denom $COSMOS_TOKEN_DENOM -o json)
cosmos_balance=$(jq -r '.amount' <<< "$cosmos_balance_json")

if [ "$cosmos_balance" -le "$COSMOS_TOKEN_TRANSFER_AMOUNT" ]; then
  echo "Balance is NOT sufficient. Exiting..."
  exit 1
fi

echo "Starting IBC transfer from Cosmoshub to Osmosis ..."
cosmos-exec tx ibc-transfer transfer $COSMOS_OSMOSIS_PORT $COSMOS_OSMOSIS_CHANNEL $OSMOSIS_ADDRESS ${COSMOS_TOKEN_TRANSFER_AMOUNT}${COSMOS_TOKEN_DENOM} --from $COSMOS_ADDRESS --yes
...
kubectl cp "${POOL_CONFIG_FILE}" osmosislocal-genesis-0:"${POOL_CONFIG_DEST}" -c validator
