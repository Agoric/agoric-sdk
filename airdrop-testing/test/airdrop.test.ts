...
const prepareKeyring = async ({ setupTestKeys }, walletCount = 10) => {
  const keyring = await setupTestKeys(
...
test.before(async t => {
  const { deleteTestKeys, setupTestKeys, startContract, ...setup } =
    await commonSetup(t);
  const keyring = prepareKeyring({ setupTestKeys });
  t.log('keyring', keyring);

  await deleteTestKeys(accounts).catch();

  const wallets = await setupTestKeys(accounts);
  t.context = { ...setup, setupTestKeys, wallets, deleteTestKeys };
