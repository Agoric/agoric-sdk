...
export default async function deployMain(progname, rawArgs, powers, opts) {
  ...
  const executeModule = async (
    moduleFile,
    { makeDefaultLeader, cache, bootP, pluginDir, pluginManager } = {},
  ) => {
  ...
  if (!opts.hostport) {
    assert.typeof(args[0], 'string', X`You must specify a script to run`);
    console.debug('Running script without CapTP!');
    const moduleFile = path.resolve(process.cwd(), args[0]);
    return executeModule(moduleFile);
  }

  const match = opts.hostport.match(/^(.*):(\d+)$/);
  host = match ? match[1] : 'localhost';
  port = match ? match[2] : '8000';

  wsurl = opts.hostport?.includes('//')
    ...
    ws.on('open', async () => {
      ...
      try {
        ...
        for (const arg of args) {
          ...
          await executeModule(moduleFile, {
            makeDefaultLeader,
            cache,
            bootP,
            pluginDir,
