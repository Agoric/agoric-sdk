...
import { startAirdrop, makeTerms } from '../../src/airdrop.proposal.js';
import {
...
const startLocalInstance = async (
  ...
  const { zoe, bundle, testFeeBrand, testFeeIssuer } = t.context;
  ...
  const { instance, ...facets } = await E(zoe).startInstance(
    installation,
    { Fee: testFeeIssuer },
    harden({
      ...customTerms,
  ...
  return { ...facets, instance, installation, timer, publicFacet };
};
...
test('mint handoff', async t => {
  ...
  t.truthy(
    tokenMint,
    'E(creatorFacet).getBankAssetMint should return a token mint',
  );
...
test('pause method', async t => {
  const { zoe: zoeRef, faucet, bundle } = await t.context;
  console.log(t.context);
  const { instance, publicFacet, timer, creatorFacet } =
    await startLocalInstance(t, bundle, {
      issuers: { Fee: await E(zoeRef).getFeeIssuer() },
  ...
  await E(creatorFacet).pauseContract();
