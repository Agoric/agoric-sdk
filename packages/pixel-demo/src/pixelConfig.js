 ...
 */
function makePixelConfig(makeUseObj, canvasSize = 10, parentAssay = undefined) {
  ...
  function prepareChildMint(assay) {
    if (childMint === undefined) {
      const makePixelConfigChild = makePixelConfig(
        makeUseObj,
        canvasSize,
        assay,
      );
  ...
  function makeMintTrait({ assay, unitOps, mintKeeper }) {
    return _coreMint =>
      harden({
