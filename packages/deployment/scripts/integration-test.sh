...
if [ -d "$loadgen" ]
then
  "$AGORIC_SDK_PATH/packages/deployment/scripts/setup.sh" show-config > "$RESULTSDIR/network-config"
  ...
  SDK_BUILD=0 MUST_USE_PUBLISH_BUNDLE=1 SDK_SRC=$AGORIC_SDK_PATH OUTPUT_DIR="$RESULTSDIR" ./start.sh \
