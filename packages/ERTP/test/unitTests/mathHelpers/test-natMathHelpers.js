...
test('natMathHelpers make', t => {
  ...
  t.throws(() => m.make(mockBrand, 4), {
    message: 'value 4 must be a bigint or copyBag, not "number"',
  });
  t.throws(
    // @ts-expect-error deliberate invalid arguments for testing
    () => m.make(mockBrand, 'abc'),
    {
      message: 'value "abc" must be a bigint or copyBag, not "string"',
    },
    `'abc' is not a nat`,
  );
  t.throws(
    ...
    () => m.make(mockBrand, -1),
    {
      message: 'value -1 must be a bigint or copyBag, not "number"',
    },
