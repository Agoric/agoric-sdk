...
const assertValueGetAssetKind = value => {
  ...
  throw Fail`value ${value} must be a bigint, copySet, copyBag, or an array, not ${passStyle}`;
};
...
const optionalBrandCheck = (allegedBrand, brand) => {
  if (brand !== undefined) {
    ...
    allegedBrand === brand ||
      Fail`amount's brand ${allegedBrand} did not match expected brand ${brand}`;
  }
};
...
const checkLRAndGetHelpers = (leftAmount, rightAmount, brand = undefined) => {
  ...
  leftBrand === rightBrand ||
    Fail`Brands in left ${leftBrand} and right ${rightBrand} should match but do not`;
