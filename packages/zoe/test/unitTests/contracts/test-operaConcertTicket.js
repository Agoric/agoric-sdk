...
The Opera is told about the show being sold out. It gets all the moolas from the sale`, async t => {
  ...
  const bobPartFinished = Promise.all([contractReadyP, alicePartFinished]).then(([{publicAPI}]) => {
    // === Bob part ===
    const ticketIssuer = publicAPI.getTicketIssuer();
    const ticketAmountMath = ticketIssuer.getAmountMath();
    ...
    return inviteIssuer.getAmountOf(bobInvite).then(({
      ...
      return zoe.redeem(bobInvite, bobProposal, {
        Money: bobFirstPaymentForTicket,
      }).then(({
        seat: { performExchange },
        payout: payoutP,
      }) => {
        t.throws(performExchange, 'performExchange from Bob should throw')
        ...
        .then(firstBobPayout => {
          ...
          .then(([bobFirstTicketAmount, bobFirstRefundAmount]) => {
            ...
            .then(() => {
              ...
              const bobSecondPaymentForTicket = bobPurse.withdraw(moola(2 * 22));
      
              return zoe.redeem(bobInvite2, bobSecondProposal, {
  ...
  .then(([{publicAPI, _operaPayouts}]) => {
    ...
    return Promise.all(_operaPayouts).then(allPayouts => {
    ...
    .then(() => {
      t.equal(operaPurse.getCurrentAmount().extent, 3 * 22, `The Opera should get ${3 * 22} moolas from ticket sales`)
    })
  })
  .catch(err => {
