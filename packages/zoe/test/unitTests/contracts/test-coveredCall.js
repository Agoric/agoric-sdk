...
test('zoe - coveredCall', async t => {
  ...
  try {
    ...
    const { payout: alicePayoutP, outcome: option } = await zoe.offer(
      aliceInvite,
      aliceProposal,
      alicePayments,
    );
...
test(`zoe - coveredCall - alice's deadline expires, cancelling alice and bob`, async t => {
  ...
  try {
    ...
    Promise.resolve(bobOutcome).then(
      outcome => t.fail(`should have been broken ${outcome}`),
      reason =>
        t.throws(_ => {
          throw reason;
...
test('zoe - coveredCall with coveredCall for invite', async t => {
  ...
  try {
    ...
    const { payout: bobPayoutP, outcome: inviteForDave } = await zoe.offer(
    ...
    const {
      ...
      outcome: daveSecondCoveredCallOutcome,
    } = await zoe.offer(
      daveExclOption,
    ...
    const {
      ...
      outcome: daveFirstCoveredCallOutcome,
    } = await zoe.offer(
