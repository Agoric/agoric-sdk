...
test(`fee charged for install, startInstance, offer, getPublicFacet`, async t => {
  ...
  const { zoeService, initialFeeFunds, getFeeCollectionPurse } = makeZoeKit(
    fakeVatAdmin.admin,
    undefined,
    feeIssuerConfig,
    zoeFeesConfig,
  ...
  const collectionPurse = getFeeCollectionPurse();
  ...
  t.true(
    AmountMath.isEqual(await E(collectionPurse).getCurrentAmount(), allFees),
  );
});
