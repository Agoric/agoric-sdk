...
test(`storeLocalIssuerRecord`, async t => {
  const { storeLocalIssuerRecord, getMathKind } = makeIssuerStorage();
  const { currencyKit } = setupIssuersForTest();
  ...
  const issuerRecord = await storeLocalIssuerRecord(localIssuerRecord);

  t.deepEqual(issuerRecord, localIssuerRecord);
