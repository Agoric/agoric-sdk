 ...
 *   The Notifier that provides notifications that periods have passed. Since
 *   notifiers can't be relied on to produce an output every time they should,
 *   we'll track the time of last payment, and catch up if any times have been
 *   missed. Compound interest will be calculated using the interestRate.
 */

/**
 * @typedef {Instance} AutoswapInstance The running contract instance for an
 *   Autoswap installation. The publicFacet from the Autoswap instance is used
 *   for producing an invitation to sell the collateral on liquidation.
 */
 ...
 *
 *   Used for getting the current value of collateral and setting liquidation
 *   triggers.
 ...
 *
 *   The ZCFSeat holding the collateral in escrow after the borrower escrows it
 * @property {() => Amount<'nat'>} getDebt
 *
 *   A function to get the current debt
 * @property {PromiseRecord<PriceQuote>} liquidationPromiseKit
 *
 *   PromiseKit that includes a promise that resolves to a PriceQuote when
 ...
 *
 *   A function to calculate the interest, given the debt value and an interest
 ...
 * @property {import('@agoric/time').Timestamp} basetime The starting point from
 ...
 *
 *   Make an invitation to close the loan by repaying the debt (including
 *   interest).
 ...
 *
 *   Get a promise for a priceQuote that will resolve if liquidation occurs. The
 *   priceQuote is for the value of the collateral that triggered the
 *   liquidation. This may be lower than expected if the price is moving
 *   quickly.
 ...
 *
 *   Get a Notifier that will be updated when the current debt (an Amount with the
 *   Loan Brand) changes. This will increase as interest is added.
