...
const start = zcf => {
   ...
   * expiration: Timestamp, value: Value}>} */
  ...
  const vote = seat => {
    ...
    attestationValue.forEach(({ expiration, handle, value }) => {
      if (storedAttestations.has(handle)) {
        ...
        if (expiration > priorAttestation.expiration) {
          storedAttestations.set(handle, { seat, expiration, value });
        }
      } else {
        storedAttestations.init(handle, harden({ seat, expiration, value }));
      }
    });
  ...
  const creatorFacet = Far('creatorFacet', {
    closeVote: currentTime => {
      ...
      const mockUnderlyingBrand = /** @type {Brand} */ (Far(
        'mockUnderlyingBrand',
        {},
      ));

      storedAttestations.values().forEach(({ seat, expiration, value }) => {
        // Only add the weight if the expiration is later than the
        // currentTime.
        if (expiration > currentTime) {
          const weight = AmountMath.make(mockUnderlyingBrand, value);
