 ...
 * @property {(result: R) => Promise<void>} pushResult Rather than waiting for
...
/**
 * @typedef {Pick<
 *   import('./priceAggregator').PriceAggregatorContract,
 ...
 * @property {(query: OracleQuery) => ERef<unknown>} query Make an unpaid query
 */
 ...
 *   contract instance
 * @property {() => AmountKeywordRecord} getCurrentFees Get the current fee
 *   amounts
 * @property {OracleCreatorFacetMakeWithdrawInvitation} makeWithdrawInvitation
 *   Create an invitation to withdraw fees
 * @property {() => Promise<Invitation>} makeShutdownInvitation Make an
 *   invitation to withdraw all fees and shutdown
 */
 ...
 * @property {(query: OracleQuery, reason: unknown) => void} onError Notice an
 *   error
 * @property {(
 *   query: OracleQuery,
 *   reply: unknown,
