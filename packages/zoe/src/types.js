 ...
 * @typedef {H & import('@endo/marshal').Remotable} Handle A type constructor
 *   for an opaque type identified by the H string. This uses an intersection
 *   type ('MyHandle' & {}) to tag the handle's type even though the actual
 *   value is just an empty object.
 */

/**
 * @typedef {string} Keyword
 * @typedef {Handle<'Invitation'>} InvitationHandle - an opaque handle for an
 *   invitation
 ...
 * @property {IssuerKeywordRecord} issuers - record with keywords keys, issuer
 *   values
 * @property {BrandKeywordRecord} brands - record with keywords keys, brand
 *   values
