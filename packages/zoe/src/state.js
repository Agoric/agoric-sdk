...
const makeIssuerTable = (withPurses = true) => {
  ...
  const makeCustomMethods = table => {
    ...
    function buildTableEntryAndPlaceHolder(issuer) {
      /**
       * a record containing one or more promises.
       * @type {[PromiseLike<AmountMath>, PromiseLike<Purse> | undefined]}
       */
      const promiseRecord = [makeLocalAmountMath(issuer), undefined];
      if (withPurses) {
        promiseRecord[1] = E(issuer).makeEmptyPurse();
      }
      const synchronousRecordP = Promise.all(promiseRecord).then(
        ([amountMath, purse]) => {
          const brand = amountMath.getBrand();
          const issuerRecord = {
            brand,
            issuer,
            amountMath,
