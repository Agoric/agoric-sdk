...
const makeZoe = (additionalEndowments = {}) => {
  ...
  const {
    ...
    brand: inviteBrand,
  } = produceIssuers('zoeInvite', 'invite');
  ...
  const makeContractFacet = instanceHandle => {
    const contractFacet = harden({
      ...
      // when the issuers are added and ready.
      addIssuers: issuers =>
        issuerTable.getPromiseForIssuerRecords(issuers).then(_ => undefined),
  ...
  const zoeService = harden({
    ...
    makeInstance: (installationHandle, terms) => {
      ...
      const storeContractInstance = ({
        ...
        return issuerTable
          .getPromiseForIssuerRecords(contractTerms.issuers)
          .then(issuerRecords => {
            const finalIssuers = issuerRecords.map(record => record.issuer);
      ...
      return issuerTable
        .getPromiseForIssuerRecords(terms.issuers)
