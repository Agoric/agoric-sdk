...
const makeCapTPMaker = (makerOpts = {}) => {
   ...
   * @property {WeakMap<any, string>} valToSlot
   */
  /**
   ...
   */
  function makeCapTP(ourId, rawSend, bootstrapObj = undefined, opts = {}) {
    const {
      onReject = err => console.error('CapTP', ourId, 'exception:', err),
      valToSlot = new WeakMap(), // exports looked up by val
    } = opts;
    ...
    function convertValToSlot(val) {
      if (syncValToSlot.has(val)) {
        // We have a synchronously-available slot.
        return syncValToSlot.get(val);
      }
 ...
 * Create a CapTP connection.
 */
export const makeCapTP = makeCapTPMaker();
