...
import { defineKind } from '@agoric/swingset-vat/src/storeModule';
  ...
  const makePool = defineKind(
    ...
    (liquidityZcfMint, poolSeat, secondaryBrand) => {
      ...
      return {
        ...
        updater,
        notifier,
        vPool: undefined, // Will close the cycle
    ...
    state => {
      ...
      const addLiquidityActual = (
        ...
        // addLiquidity can't be called until the pool has been created.
        // We verify that the asset is NAT before creating a pool.
        ...
        if (feeSeat) {
          zcf.reallocate(state.poolSeat, zcfSeat, feeSeat);
        } else {
      ...
      const pool = {
        ...
        getVPool: () => state.vPool,
      };
      return pool;
    },
    // Close the cycle
    (state, pool) => {
      state.vPool = makeSinglePool(
        zcf,
        pool,
        getProtocolFeeBP,
        getPoolFeeBP,
      ...
      const getInputPriceForPA = (amountIn, brandOut) =>
        state.vPool.externalFacet.getInputPrice(
          amountIn,
