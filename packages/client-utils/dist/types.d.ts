...
import type { Installation, Instance } from '@agoric/zoe/src/zoeService/types.js';
 ...
 */
export type TypedPublished<T extends string> = T extends keyof PublishedTypeMap ? PublishedTypeMap[T] : T extends `wallet.${string}.current` ? CurrentWalletRecord : T extends `wallet.${string}` ? UpdateRecord : T extends `ymax${'0' | '1'}.portfolios.portfolio${number}` ? StatusFor['portfolio'] : T extends `ymax${'0' | '1'}.portfolios.portfolio${number}.positions.${string}` ? StatusFor['position'] : T extends `ymax${'0' | '1'}.portfolios.portfolio${number}.pendingTx.tx${number}` ? StatusFor['pendingTx'] : T extends `ymax${'0' | '1'}.portfolios.portfolio${number}.flows.flow${number}` ? StatusFor['flow'] : T extends `ymax${'0' | '1'}.portfolios.portfolio${number}.flows.flow${number}.steps` ? StatusFor['flowSteps'] : T extends `ymax${'0' | '1'}.evmWallets.0x${string}.portfolio` ? StatusFor['evmWalletPortfolios'] : T extends `ymax${'0' | '1'}.evmWallets.0x${string}` ? StatusFor['evmWallet'] : T extends `committees.${string}.latestQuestion` ? QuestionDetails : T extends `committees.${string}.latestOutcome` ? OutcomeRecord : T extends `vaultFactory.managers.manager${number}.metrics` ? VaultDirectorMetrics : T extends `fastUsdc.txns.${string}` ? TransactionRecord : unknown;
export {};
