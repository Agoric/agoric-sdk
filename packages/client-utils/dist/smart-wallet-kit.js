...
import { getOfferResult, getOfferWantsSatisfied, makeWalletStateCoalescer, } from './smart-wallet-utils.js';
/**
 ...
 * @alpha
 */
export const makeSmartWalletKitFromVstorageKit = async (vsk, { names = true } = {}) => {
    ...
    const storedWalletState = async (from, minHeight = undefined) => {
        const history = await vsk.vstorage.readFully(`published.wallet.${from}`, minHeight);
     ...
     */
    const pollOffer = async (from, id, minHeight, untilNumWantsSatisfied = false) => {
...
//# sourceMappingURL=smart-wallet-kit.js.map