...
const isSequenceNumberMismatch = (error) => 
 ...
 * @alpha
 */
export const makeSequencingSmartWallet = (signingSmartWalletKit, txSequencer, { log = () => { } }) => {
    ...
    const { query, address, executeOffer: _unsafeExecuteOffer, sendBridgeAction: unsafeSendBridgeAction, ...swk } = signingSmartWalletKit;
    ({
        query,
        address,
     ...
     */
    const runResyncable = async (label, thunk) => {
        await null;
        try {
            return await thunk();
        }
     ...
     */
    const sendBridgeAction = async (action, fee, memo, manualSignerData) => {
     ...
     */
    const executeOffer = async (offer, fee, memo, manualSignerData) => {
        manualSignerData === undefined || Fail `manual signerData is not supported`;
        const txId = offer.offerArgs?.txId;
        const label = `executeOffer${txId ? ` ${txId}` : ''}`;
        return enqueue(label, async () => {
            const signerData = makeSignerData(label);
            const action = { method: 'executeOffer', offer };
...
//# sourceMappingURL=smart-wallet-with-sequence.js.map