    ...
    const update = updateRecord => {
        ...
        switch (updateRecord.updated) {
            ...
            case 'offerStatus': {
                ...
                if (status.invitationSpec.source === 'purse' &&
                    ...
                    if (receptionRecord) {
                        ...
                        throw Error(`no record of invitation in offerStatus ${JSON.stringify(status)}`);
                    }
                }
                break;
            }
 ...
 */
const findUpdate = (id, getLastUpdate, retryOpts, isMatch) => retryUntilCondition(getLastUpdate, isMatch, `${id}`, retryOpts);
/**
...
export const getInvocationUpdate = async (id, getLastUpdate, retryOpts) => {
    ...
    const isMatch = update => update.updated === 'invocation' &&
        update.id === id &&
        !!(update.error || update.result);
    const found = /** @type {UpdateRecord & { updated: 'invocation' }} */ (await findUpdate(id, getLastUpdate, retryOpts, isMatch));
    if (found.error)
...
export const getOfferResult = async (id, getLastUpdate, retryOpts) => {
    ...
    /** @type {(update: UpdateRecord) => boolean} */
    const isMatch = update => update.updated === 'walletAction' ||
        (update.updated === 'offerStatus' &&
            update.status.id === id &&
            !!(update.status.error || update.status.result));
    const found = 
    /** @type {UpdateRecord & { updated: 'walletAction' | 'offerStatus' }} */ (await findUpdate(id, getLastUpdate, retryOpts, isMatch));
...
//# sourceMappingURL=smart-wallet-utils.js.map