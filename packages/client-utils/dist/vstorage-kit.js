import { boardSlottingMarshaller, makeBoardRemote, } from '@agoric/internal/src/marshal/board-client-utils.js';
...
export const storageHelper = {
    ...
    unserializeTxt: (txt, ctx) => {
        ...
        return capDatas.map(capData => boardSlottingMarshaller(ctx.convertSlotToVal).fromCapData(capData));
    },
...
export const makeAgoricNames = async (ctx, vstorage) => {
    ...
    const reverse = {};
    const entries = await Promise.all(['brand', 'instance', 'vbankAsset'].map(async (kind) => {
        const content = await vstorage.readLatest(`published.agoricNames.${kind}`);
 ...
 * @alpha
 */
export const makeVstorageKitFromVstorage = ({ vstorage, networkConfig, marshaller, }) => {
    /** @type {IdMap} */
    ...
    /** @type {Promise<any>} */ (vstorage.readLatest(path).then(unserializeHead));
    /**
    ...
    /** @type {Promise<TypedPublished<any>>} */ (readLatestHead(`published.${subpath}`));
    return {
        fromBoard,
        marshaller,
        networkConfig,
...
export const makeVstorageKit = ({ fetch }, networkConfig) => {
    const vstorage = tryNow(() => makeVStorage({ fetch }, networkConfig), err => {
...
//# sourceMappingURL=vstorage-kit.js.map