...
import { QueryChildrenRequest, QueryChildrenResponse, QueryDataRequest, QueryDataResponse, } from '@agoric/cosmic-proto/agoric/vstorage/query.js';
/**
 ...
 */
export const makeAbciQuery = (path = 'published', { kind = /** @type {T} */ ('children'), height = 0 } = {}) => {
...
export const makeVStorage = ({ fetch }, config) => {
     ...
     */
    const getVstorageJson = async (vstoragePath, { kind = /** @type {T} */ ('children'), height = 0 } = {}) => {
        const url = config.rpcAddrs[0] + makeAbciQuery(vstoragePath, { kind, height });
     ...
     */
    const readStorageMeta = async (path = 'published', { kind = /** @type {T} */ ('children'), height = 0 } = {}) => {
        await null;
        const rpc = kindToRpc[kind];
        ...
        if (response?.code !== 0) {
            /** @type {any} */
            const err = Error(`error code ${response?.code} reading ${kind} of ${path}: ${response.log}`);
        ...
        const metaResponseBase = {
            blockHeight: response.height === undefined ? undefined : BigInt(response.height),
            log: response.log,
        };
    ...
    const vstorage = {
        ...
        async readFully(path, minHeight = undefined) {
            ...
            do {
                ...
                let values;
                try {
                    ({ blockHeight, values } = await vstorage.readAt(path, blockHeight === undefined ? undefined : Number(blockHeight) - 1));
                ...
                if (minHeight && Number(blockHeight) <= Number(minHeight))
                    break;
