 ...
 */
export const sleep = (ms, { log = () => { }, setTimeout }) => new Promise(resolve => {
 ...
 * @returns {Promise<T>}
 */
export const retryUntilCondition = async (operation, condition, message, { maxRetries = 6, retryIntervalMs = 3500, reusePromise = false, renderResult = x => x, 
// XXX mixes ocaps with configuration options
log = console.log, setTimeout, }) => {
    ...
    while (retries < maxRetries) {
        ...
        retries += 1;
        log(`Retry ${retries}/${maxRetries} - Waiting for ${retryIntervalMs}ms for ${message}...`);
...
const makeGetInstances = follow => async () => {
    const instanceEntries = await follow('-lF', `:published.agoricNames.instance`);
 ...
 * @param {WaitUntilOptions} options
 */
export const waitUntilContractDeployed = (contractName, ambientAuthority, options) => {
    ...
    return retryUntilCondition(getInstances, instanceObject => Object.keys(instanceObject).includes(contractName), errorMessage, { log, setTimeout, ...resolvedOptions });
};
/// ////////// Making sure an account is successfully funded with vbank assets like IST, BLD etc. ///////////////
...
export const waitUntilAccountFunded = (destAcct, io, threshold, options) => {
    ...
    const { errorMessage, ...resolvedOptions } = overrideDefaultOptions(options);
    return retryUntilCondition(async () => queryCosmosBalance(destAcct), balances => checkCosmosBalance(balances, threshold), errorMessage, { log, setTimeout, ...resolvedOptions });
};
...
const checkOfferState = (offerStatus, waitForPayouts, offerId) => {
    ...
    if (updated !== 'offerStatus')
        return false;
    if (!status)
        return false;
    if (status.id !== offerId)
        return false;
    if (!status.numWantsSatisfied || status.numWantsSatisfied !== 1)
        return false;
    if (waitForPayouts && status.payouts)
        return true;
    if (!waitForPayouts && status.result)
        return true;
    return false;
};
/**
 ...
 * @param {WaitUntilOptions} options
 */
export const waitUntilOfferResult = (addr, offerId, waitForPayouts, io, options) => {
    const { log, follow, setTimeout } = io;
    const queryWallet = makeQueryWallet(follow);
    const { errorMessage, ...resolvedOptions } = overrideDefaultOptions(options);
    return retryUntilCondition(async () => queryWallet(addr), status => checkOfferState(status, waitForPayouts, offerId), errorMessage, { log, reusePromise: true, setTimeout, ...resolvedOptions });
};
/// ////////// Making sure a core-eval successfully sent zoe invitations to committee members for governance /////////////
/**
 *
...
const checkForInvitation = update => {
    ...
    if (updated !== 'balance')
        return false;
    if (!currentAmount || !currentAmount.brand)
        return false;
...
export const waitUntilInvitationReceived = (addr, io, options) => {
    ...
    const queryWallet = makeQueryWallet(follow);
    const { errorMessage, ...resolvedOptions } = overrideDefaultOptions(options);
    return retryUntilCondition(async () => queryWallet(addr), checkForInvitation, errorMessage, { log, reusePromise: true, setTimeout, ...resolvedOptions });
};
/// ////////// Making sure an offer was exited successfully /////////////
const makeQueryWalletCurrent = follow => (/** @type {string} */ addr) => follow('-lF', `:published.wallet.${addr}.current`);
/**
 * @param {object} update
 * @param {string} offerId
 * @returns {boolean}
...
export const waitUntilOfferExited = async (addr, offerId, io, options) => {
    ...
    const { errorMessage, ...resolvedOptions } = overrideDefaultOptions(options);
    return retryUntilCondition(async () => queryWalletCurrent(addr), update => checkLiveOffers(update, offerId), errorMessage, { log, setTimeout, ...resolvedOptions });
};
...
const fetchLatestEcQuestion = async (basePath, vstorage) => {
    ...
    const [latestOutcome, latestQuestion] = await Promise.all([
        /** @type {Promise<ElectionResult["latestOutcome"]>} */ (vstorage.readLatestHead(pathOutcome)),
        /** @type {Promise<ElectionResult["latestQuestion"]>} */ (vstorage.readLatestHead(pathQuestion)),
    ]);
...
const checkCommitteeElectionResult = (electionResult, expectedResult) => {
    const { latestOutcome: { outcome, question }, latestQuestion: { closingRule: { deadline }, questionHandle, }, } = electionResult;
    const { outcome: expectedOutcome, deadline: expectedDeadline } = expectedResult;
 ...
 * @param {WaitUntilOptions} options
 */
export const waitUntilElectionResult = (committeePathBase, expectedResult, io, options) => {
    const { vstorage, log, setTimeout } = io;
    const { maxRetries, retryIntervalMs, errorMessage } = overrideDefaultOptions(options);
    return retryUntilCondition(() => fetchLatestEcQuestion(committeePathBase, vstorage), electionResult => checkCommitteeElectionResult(electionResult, expectedResult), errorMessage, { maxRetries, retryIntervalMs, log, setTimeout });
};
