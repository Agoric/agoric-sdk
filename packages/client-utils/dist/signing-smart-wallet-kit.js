...
import { MsgWalletSpendAction } from '@agoric/cosmic-proto/codegen/agoric/swingset/msgs.js';
import { TxRaw } from '@agoric/cosmic-proto/codegen/cosmos/tx/v1beta1/tx.js';
 ...
 * @alpha
 */
export const makeSigningSmartWalletKitFromClient = async ({ smartWalletKit: walletUtils, address, client, }) => {
    ...
    const query = {
        ...
        pollOffer: (...args) => swk.pollOffer(address, ...args),
    };
    const sendBridgeAction = async (action, fee = defaultFee, memo = '', signerData) => {
        ...
        const signedTx = await client.sign(address, messages, fee, memo, signerData);
        const txBytes = TxRaw.encode(signedTx).finish();
        return client.broadcastTx(txBytes);
    };
    const executeOffer = async (offer, fee, memo, signerData) => {
        const offerP = swk.pollOffer(address, offer.id);
        // Await for rejection handling
        await sendBridgeAction({ method: 'executeOffer', offer }, fee, memo, signerData);
        return offerP;
    };
    return harden({
        ...swk,
 ...
 * Augment a read-only SmartWalletKit with signing ability
 */
export const makeSigningSmartWalletKit = async ({ connectWithSigner, walletUtils, }, MNEMONIC) => {
...
//# sourceMappingURL=signing-smart-wallet-kit.js.map