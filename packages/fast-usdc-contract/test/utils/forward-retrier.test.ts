...
test('stops retrying if route becomes derelict during clearing', t => {
  ...
  triggerRouteEvent('onWorking', route); // Could also be onFailure
...
test('retries on onFailure if route is still working', t => {
  ...
  triggerRouteEvent('onFailure', route);
...
test('does not retry on onFailure if route becomes derelict', t => {
  const { routeHealth, forwardFunds, failedForwardsMap, triggerRouteEvent } =
    setup(t);
