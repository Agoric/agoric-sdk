...
const outputFile = { writeText: async (_text: string) => undefined };
const cwd = {
  join: (path: string) => {
    if (path !== 'invite.json') throw Error(`unexpected output path ${path}`);
    return outputFile;
  },
} as never;

test('instrument oracle invitation supports four explicit modes', t => {
  t.deepEqual(parseInstrumentOracleArgs(['ymax0', '--send', '--accept'], cwd), {
    contract: 'ymax0',
    mode: 'send-and-accept',
  });
  t.deepEqual(
    parseInstrumentOracleArgs(
      ['ymax0', '--send', '--to', 'agoric1operator'],
      cwd,
    ),
    {
      contract: 'ymax0',
  ...
  t.deepEqual(parseInstrumentOracleArgs(['ymax1', '--accept'], cwd), {
    contract: 'ymax1',
    mode: 'accept-only',
  });
  t.throws(
    () =>
      parseInstrumentOracleArgs(
        ['ymax1', '--generate', '--to', 'agoric1operator'],
        cwd,
      ),
