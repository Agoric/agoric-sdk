...
const generateAuthzOperatorUpgrade = async (
  ...
  const upgradeSigner = await makeUpgradeSigner(upgradeTarget);
  const request = await upgradeSigner.signUpgrade({
    bundleId: install.bundleId,
    invocationId,
    memo: '',
...
const expectMissingAny = (cause: unknown, messages: string[]) => {
  if (cause instanceof Error && messages.includes(cause.message)) {
    return;
  }
  throw cause;
};
