...
const Usage = `invite-instrument-oracle ymax1|ymax0 (--send [--accept] | --generate | --accept) [--to agoric1...] [--output tx.json]`;
...
export const parseInstrumentOracleArgs = (scriptArgs: string[]): ParsedArgs => {
...
const generateInvitationTx = async (
  ...
  { walletKit, cwd }: Pick<RunTools, 'walletKit' | 'cwd'>,
) => {
...
const inviteInstrumentOracle = async (tools: RunTools) => {
  await null;
  const args = parseInstrumentOracleArgs(tools.scriptArgs);
