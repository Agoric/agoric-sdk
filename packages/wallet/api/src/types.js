 ...
 *
 * @typedef {WalletUser & {
 *   getAdminFacet: () => WalletAdminFacet;
 ...
 *   exposed via the iframe/WebSocket bridge that a Dapp UI can use to access
 *   the wallet.
 ...
 *   This notifier should only contain information that is safe to pass to the
 *   dapp UI.
 * @property {() => Promise<ZoeService>} getZoe Get the Zoe Service
 * @property {() => Promise<import('@agoric/vats').Board>} getBoard Get the
 *   Board
 * @property {(...path: Array<unknown>) => Promise<unknown>} getAgoricNames Get
 *   the curated Agoric public naming hub
 * @property {(...path: Array<unknown>) => Promise<unknown>} getNamesByAddress
 *   Get the Agoric address mapped to its public naming hub
 * @property {(brands: Array<Brand>) => Promise<Array<Petname>>} getBrandPetnames
