...
test('two parameters', async t => {
  const { zcf } = await makeZoeKit(drachmaKit);
  const paramManager = await makeBuilder(zcf)
...
test('Amount', async t => {
  ...
  const { zcf } = await makeZoeKit(waxKit);
  const paramManager = await makeBuilder(zcf)
...
test('Invitation', async t => {
  ...
  const paramManagerBuilder = await makeBuilder(zcf)
    .addBrand('Collateral', drachmaBrand)
    .addAmount('Amt', drachmaAmount)
    .addInvitation('Invite', [invitation, invitationAmount]);
