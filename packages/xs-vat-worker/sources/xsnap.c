...
static size_t readNetString(FILE *inStream, char** dest);
static void writeNetString(FILE* outStream, char* buf, size_t len);
...
int main(int argc, char* argv[]) 
{
	...
	xsBeginMetering(machine, fxMeteringCallback, interval);
	{
		...
		while (!done) {
			char* nsbuf;
			size_t nslen = readNetString(fromParent, &nsbuf);
			if (!nslen) {
				if (feof(fromParent)) {
					break;
				} else {
			...
			case 'e':
				...
				xsBeginHost(machine);
				{
					...
					xsTry {
						if (command == 'd') {
							xsCall1(xsGlobal, xsID("onMessage"), xsVar(1));
						} else {
			...
			case 'w':
				...
				if (snapshot.error == 0) {
					...
					// TODO: dynamically build error message including strerror(...)
					writeNetString(toParent, "!", 1);
				}
				break;
			case -1:
 ...
 * @returns count of bytes read or 0 for error;
...
static size_t readNetString(FILE *inStream, char** dest)
{
    ...
    if (fscanf(inStream, "%9lu", &len) < 1) {
		...
		err = "Cannot read netstring, reading length prefix, fscanf";
	} else if (fgetc(inStream) != ':') {
		err = "Cannot read netstring, invalid delimiter or end of file, fgetc";
	} else {
		buf = malloc(len + 1); /* malloc(0) is not portable */
		if (!buf) {
			err = "Cannot read netstring, cannot allocate message buffer, malloc";
		} else if (fread(buf, 1, len, inStream) < len) {
			err = "Cannot read netstring, cannot read message body, fread";
		} else if (fgetc(inStream) != ',') {
			err = "Cannot read netstring, cannot read trailer, fgetc";
		} else {
			*(buf + len) = 0;
		}
		if (err) {
...
static void writeNetString(FILE* outStream, char* buf, size_t length)
{
	char* err = NULL;

	if (fprintf(outStream, "%lu:", length) < 1) {
	  err = "Cannot write netstring, error writing length prefix";
	} else if (fwrite(buf, 1, length, outStream) < length) {
	  err = "Cannot write netstring, error writing message body";
	} else if (fputc(',', outStream) == EOF) {
	  err = "Cannot write netstring, error writing terminator";
	} else {
		fflush(outStream);
	}
}
...
/**
 * sysCall: write netstring; read netstring reply
 */
static void fx_sysCall(xsMachine *the)
{
	char* err = NULL;
	...
	if (fprintf(outStream, "%lu:?", length + 1) < 1) {
		err = "Cannot write netstring, error writing length prefix";
	} else if (fwrite(buf, 1, length, outStream) < length) {
		err = "Cannot write netstring, error writing message body";
	} else if (fputc(',', outStream) == EOF) {
	...
	// read netstring
	size_t len;
	if (fscanf(fromParent, "%9lu", &len) < 1) {
		/* >999999999 bytes is bad */
		err = "Cannot read netstring, reading length prefix, fscanf";
	} else if (fgetc(fromParent) != ':') {
		err = "Cannot read netstring, invalid delimiter or end of file, fgetc";
	} else {
		buf = malloc(len); /* malloc(0) is not portable */
		if (!buf) {
			err = "Cannot read netstring, cannot allocate message buffer, malloc";
		} else if (fread(buf, 1, len, fromParent) < len) {
			err = "Cannot read netstring, cannot read message body, fread";
		} else if (fgetc(fromParent) != ',') {
			err = "Cannot read netstring, cannot read trailer, fgetc";
		} else {
