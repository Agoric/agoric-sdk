  ...
  async *subscribeAll(queries: string[]) {
    ...
    const qs = [...newQueriesSet.keys()].map(query => {
      ...
      this.#subscriptions.set(subId, {
        ...
        notified: (response: JSONRPCResponse) => {
          if (isFirstResponse) {
            // Ignore the initial empty data response.
    ...
    try {
      while (true) {
        ...
        const entry = await firstCell;
        // console.log('got next event', entry);
        firstCell = entry.tail;
        if (entry.head.error) {
          yield Promise.reject(Error(entry.head.error.message));
          continue;
        }
        yield entry.head.result;
      }
    } catch (error) {
