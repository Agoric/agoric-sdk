...
export function buildTxResponseString<T extends EncoderMessage<any>[]>(
  ...
  const encodedMessages = messages.map(({ encoder, message }) =>
    encoder.encode(encoder.fromPartial(message)).finish(),
  );

  const msgResponses = encodedMessages.map(encodedMsg =>
    Any.fromPartial({
      typeUrl: messages[0].encoder.typeUrl,
      value: encodedMsg,
    }),
  );
