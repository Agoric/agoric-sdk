...
const startLocalInstance = async (
  ...
  return { instance, installation, creatorFacet, timer, publicFacet };
};
...
test.serial('Start the contract', async t => {
  ...
  const { instance: contractinstance } = await startLocalInstance(
    t,
    bundle,
    localTestConfig,
  );

  t.log(contractinstance);
  t.is(typeof contractinstance, 'object');
});
...
test('mint handoff', async t => {
  const { zoe: zoeRef, bundle, faucet } = await t.context;
  console.log(t.context);
  const { instance, creatorFacet, timer } = await startLocalInstance(
    t,
    bundle,
    {
      zoe: zoeRef,
  ...
  const tokenMint = await E(creatorFacet).getBankAssetMint();
  t.truthy(
    tokenMint,
    'creatorFacet.getBankAssetMint should return a token mint',
  );
...
test('pause method', async t => {
  ...
  const { instance, publicFacet, timer, creatorFacet } =
    await startLocalInstance(t, bundle, {
      issuers: { Fee: await E(zoe).getFeeIssuer() },
      terms: defaultCustomTerms,
    });
  ...
  await simulateClaim(t, zoe, instance, feePurse, accounts[2]);

  await E(timer).advanceBy(oneDay);

  const adminAddress = 'agoric1jng25adrtpl53eh50q7fch34e0vn4g72j6zcml';

  const adminInvitation =
    await E(creatorFacet).makePauseContractInvitation(adminAddress);

  const adminOffer = await E(zoe).offer(adminInvitation);

  const [payout, pauseResult] = await Promise.all([
    E(adminOffer).getPayout('Tokens'),
    E(adminOffer).getOfferResult(),
  ]);
  t.deepEqual(payout, []);

  await t.throwsAsync(
    simulateClaim(t, zoeRef, instance, feePurse, accounts[3]),
    {
