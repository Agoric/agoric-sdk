...
test('restake contract schedules claimRewards() and delegate() on an interval', async t => {
  ...
  const withdrawRewardAck = messages.at(
    -2,
  ...
  t.is(
    ...
    'Account withdrew rewards',
  );

  const delegateAck = messages.at(-1) as IBCEvent<'acknowledgementPacket'>;
  t.is(
    delegateAck.acknowledgement,
    buildMsgResponseString(MsgDelegateResponse, {}),
    'Account delegated the claimed rewards',
  );
  ...
  // console.log('await inspectDibcBridge()', newMessages);
