 ...
 * @import {MockOrchestrator, MockChain, MockOrchAccount} from '../mock-orch/types.js'
...
export const prepareERTPOrchestrator = zone => {
    ...
    },
    {
      purse: {
        ...
        async deposit(payment, optAmountShape = undefined) {
          ...
          const paymentAmount = paymentLedger.get(payment);
          if (optAmountShape !== undefined) {
            mustMatch(paymentAmount, optAmountShape);
          }
  ...
  const mockERTPOrchestrator = zone.exoClass(
     ...
     * @param {MockOrchestratorAdmin} orchAdmin
     */
    (orchestrator, orchAdmin) => {
    ...
    },
    {
      ...
      async provideIssuerKitForDenom(
        ...
        const { orchestrator, orchAdmin, denom2IssuerKit, brand2Denom } =
          this.state;
        await null;
        if (!denom2IssuerKit.has(denom)) {
          ...
          try {
            ...
            ({ chain } = await E(orchestrator).getDenomInfo(denom));
            issuerKitPlus = mockIssuerKit(denom, chain);
          } catch (err) {
            chain = await E(orchestrator).getChain(defaultChainName);
            const denomMint = await E(orchAdmin).makeDenom(denom, chain);
