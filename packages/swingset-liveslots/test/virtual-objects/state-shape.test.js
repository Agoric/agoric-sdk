...
/** @import {VatSyscallResolve} from '../../src/types.js'; */
...
const dispatchForResult = async (ls, syscallLog, message) => {
  const logLen = syscallLog.length;
  lastPnum += 1;
  ...
  const [_vpid, isRejection, capdata] =
    /** @type {VatSyscallResolve} */ (
      syscallLog.slice(logLen).findLast(entry => entry.type === 'resolve')
    )?.resolutions.find(resolution => resolution[0] === resultVpid) ||
    Fail`unsettled by syscalls ${syscallLog.slice(logLen)}`;
...
test('durable stateShape must match or extend', async t => {
  ...
  // Tolerate absence of the new field "x2".
  t.deepEqual(
    kser({ x2: undefined, ...state3 }),
    kser({ x2: undefined, ...state1 }),
  );
  ...
  for (const [label, value, message] of verifications) {
    ...
    await (message ? t.throwsAsync(tryUpdate, { message }) : tryUpdate());
    const state = await dispatchForResult(ls3, log3, [objRef, 'get', fields]);
    const expectState = message ? lastState : value;
    t.deepEqual(
      kser({ x2: undefined, ...state }),
      kser({ x2: undefined, ...expectState }),
      `${label}`,
    );
    lastState = state;
  }
