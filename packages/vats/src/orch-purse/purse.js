...
import { makeAmountStore } from '@agoric/ertp/src/amountStore.js';
import { makeTransientNotifierKit } from '@agoric/ertp/src/transientNotifier.js';

/**
 * @import {AssetKind, RecoverySetsOption, Brand, Payment} from '@agoric/ertp'
 *
 * @import {MinChain} from './types.js'
 */
...
export const prepareOrchPurseKind = (
  ...
  const makeOrchPurseKit = issuerZone.exoClassKit(
    `${name} OrchPurse`,
    OrchPurseIKit,
    () => {
      const orchAccount = orchChain.makeAccount();
  ...
  return () => makeOrchPurseKit().purse;
};
harden(prepareOrchPurseKind);
