    ...
    E(port).addListener(
      Far('listener', {
        async onAccept(_port, _localAddr, _remoteAddr, _listenHandler) {
          ...
          const proxyMethod = name => (...args) => E(chandlerP)[name](...args);
 ...
 *
 * @param {{ argv?: { bootMsg: typeof BootMsgEx }} | undefined } vatParameters
 * @param {{
 *   bank: ERef<BankVatRoot>,
 *   board: ERef<BoardVatRoot>,
 *   mints: ERef<MintsVatRoot>,
 *   network: ERef<Protocol>,
 *   sharing: ERef<SharingVatRoot>,
 *   timer: ERef<TimerVatRoot>,
 *   priceAuthority: ERef<PriceAuthorityVatRoot>,
 *   walletManager: ERef<WalletManagerVatRoot>,
 *   zoe: ERef<ZoeVatRoot>,
 *   distributeFees: ERef<DistributeFeesVatRoot>,
 * }} vats
 * @param {import('./bridge.js').BridgeManager} bridgeManager
 * @param {unknown} timerDevice
 * @param {unknown} vatAdminSvc
 * @param {boolean} noFakeCurrencies
 *
 * @typedef {ReturnType<import('./vat-bank')['buildRootObject']>} BankVatRoot
 * @typedef {ReturnType<import('./vat-board')['buildRootObject']>} BoardVatRoot
 * @typedef {ReturnType<import('./vat-distributeFees')['buildRootObject']>} DistributeFeesVatRoot
 * @typedef {ReturnType<import('./vat-mints')['buildRootObject']>} MintsVatRoot
 * @typedef {ReturnType<import('./vat-sharing')['buildRootObject']>} SharingVatRoot
 * @typedef {ReturnType<import('./vat-priceAuthority')['buildRootObject']>} PriceAuthorityVatRoot
 * @typedef {ReturnType<import('./vat-walletManager')['buildRootObject']>} WalletManagerVatRoot
 * @typedef {ReturnType<import('./vat-zoe')['buildRootObject']>} ZoeVatRoot
 * @typedef {ReturnType<import('@agoric/swingset-vat/src/vats/vat-timerWrapper.js').buildRootObject>} TimerVatRoot
 */
async function makeChainBundler(
  ...
  const centralBootstrapPayment = await E(treasuryCreator).getBootstrapPayment(
  ...
  return Far('chainBundler', {
    async createUserBundle(_nickname, address, powerFlags = []) {
      ...
      const bundle = harden({
        ...
        feeMintAccess, // @@@@for experimenting with lien / attestation
        attestationMaker,
        agoricNames,
        bank,
        chainTimerService,
