 ...
 * @property {(...path: string[]) => Promise<any>}                lookup  Look
 *   up a path of keys starting from the current NameHub. Wait on any reserved
 *   promises.
 * @property {(includeReserved?: boolean) => [string, unknown][]} entries get
 *   all the entries available in the current NameHub
 * @property {() => string[]}                                     keys    get
 *   all names available in the current NameHub
 * @property {() => unknown[]}                                    values  get
 *   all values available in the current NameHub
 */
 ...
 * @property {(key: string) => Promise<void>}                                 reserve
 *   Mark a key as reserved; will return a promise that is fulfilled when the key
 *   is updated (or rejected when deleted). If the key was already set it does
 *   nothing.
 ...
 *   lookupAdmin.
 * @property {(...path: string[]) => Promise<NameAdmin>}                      lookupAdmin
...
/**
 * @typedef  {object}                      BridgeHandler An object that can
 *   receive messages from the bridge device
 * @property {(obj: any) => Promise<void>} fromBridge    Handle an inbound
 *   message
 */

/**
 * @typedef  {object}                                 ScopedBridgeManager An
 *   object which handles messages for a specific bridge
