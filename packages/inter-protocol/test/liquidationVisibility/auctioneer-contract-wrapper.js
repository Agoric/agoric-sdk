...
import { makeNatAmountShape } from '../../src/contractSupport.js';
import {
  makeOfferSpecShape,
  prepareAuctionBook,
} from '../../src/auction/auctionBook.js';
import { auctioneerParamTypes } from '../../src/auction/params.js';
...
export const start = async (zcf, privateArgs, baggage) => {
   ...
   * @type {MapStore<
   *   Brand,
   *   import('../../src/auction/auctionBook.js').AuctionBook
   * >}
   */
  ...
  const publicFacet = augmentPublicFacet(
    harden({
      ...
      makeBidInvitation(collateralBrand) {
        ...
        /**
         * @param {ZCFSeat} zcfSeat
         * @param {import('../../src/auction/auctionBook.js').OfferSpec} offerSpec
         */
      ...
      setRejectGetSchedules(flag) {
        rejectGetSchedules = flag;
      },
      getScheduleUpdates() {
  ...
  const creatorFacet = makeFarGovernorFacet(
    Far('Auctioneer creatorFacet', {
      ...
      /**
       * @returns {Promise<
       *   import('../../src/auction/scheduler.js').FullSchedule
       * >}
       */
      getSchedule() {
