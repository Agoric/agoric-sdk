  ...
  const makeAuctionBookKit = prepareExoClassKit(
    ...
    },
    {
      ...
      self: {
        ...
        addOffer(bidSpec, seat, trySettle) {
          ...
          // FIXME restore once the amounts are available
    ...
    },
    {
      finish: async ({ state }) => {
        ...
        // FIXME PITA that this state is async. maybe caller of makeAuctionBook should provide.
        state.collateralAmountShape = collateralAmountShape;
        state.currencyAmountShape = currencyAmountShape;
        state.scaledBidBook = makeScaledBidBook(
          makeBrandedRatioPattern(currencyAmountShape, currencyAmountShape),
          collateralBrand,
        );

        state.priceBook = makePriceBook(
...
/** @typedef {ReturnType<Awaited<ReturnType<typeof prepareAuctionBook>>>} AuctionBook */
