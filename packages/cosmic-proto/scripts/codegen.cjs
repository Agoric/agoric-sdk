...
const { Bundler, TelescopeBuilder } = require('@cosmology/telescope');
...
const createTypeUrlProto = async () => {
  const { store } = builder;
  const fileExports = {};
  ...
  for (const [importFrom, exports] of sortedEntries) {
    lastImport += 1;
    const imp = `imp${lastImport}`;
    imports[imp] = importFrom;
    for (const [typeUrl, exp] of exports) {
      props.push(`\n  ${JSON.stringify(typeUrl)}: ${imp}.${exp};`);
    }
  }
...
// DO NOT EDIT; generated by ${path.relative(process.cwd(), __filename)}
  ...
  out.push('', `export type TypeUrlProto = {`, ...props, `};`, '');
  return fsp.writeFile(path.join(outPath, 'typeUrlProto.ts'), out.join('\n'));
};

builder
  .build()
  .then(createTypeUrlProto)
  .then(() => {
