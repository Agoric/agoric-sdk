 ...
 *   An AsyncIterator that can be forked at a given position into multiple
 *   independent ForkableAsyncIterators starting from that position.
 */
 ...
 * } & ForkableAsyncIterator<T, TReturn, TNext>} ForkableAsyncIterableIterator
 */
...
/**
 * @template T
 * @typedef {object} PublicationRecord Will be shared between machines, so it
 *   must be safe to expose. But software outside the current package should
 *   consider it opaque, not depending on its internal structure.
 * @property {IteratorResult<T>} head
 * @property {bigint} publishCount starts at 1 for the first result and advances
 *   by 1 for each subsequent result
 ...
 *   immediate successor via a `tail` promise) that is later than the provided
 *   publishCount. Used to make forward-lossless ("each") iterators.
 */

/**
 * @template T
 * @typedef {ForkableAsyncIterable<T, T> & EachTopic<T>} IterableEachTopic An
 *   EachTopic with default asyncIterable behaviour.
 *
...
/**
 * @template T
 * @typedef {AsyncIterableOnly<T, T> & LatestTopic<T>} IterableLatestTopic A
 *   LatestTopic with default asyncIterable behaviour.
 *
 ...
 *   the promise promptly resolves to the current record. Otherwise, after the
 *   next state change, the promise resolves to the resulting record. This is an
 *   attenuated form of `subscribeAfter` whose return value stands alone and
 *   does not allow consumers to pin a chain of historical PublicationRecords.
 *   Used to make lossy ("latest") iterators. NOTE: Use of `number` as an
 *   `updateCount` is deprecated.
 */
...
/**
 * @template T
 * @typedef {LatestTopic<T> & EachTopic<T>} Subscriber A stream of results that
 *   allows consumers to configure forward-lossless "each" iteration with
 *   `subscribeEach` and lossy "latest" iteration with `subscribeLatest`.
 */

/**
 * @template T
 * @typedef {object} Publisher A valid sequence of calls to the methods of an
 *   `IterationObserver` represents an iteration. A valid sequence consists of
 *   any number of calls to `publish` with the successive non-final values,
 *   followed by a final call to either `finish` with a successful `completion`
 *   value or `fail` with the alleged `reason` for failure. After at most one
 ...
 *
 *   - 'mandatory' means that each value must be durable, so it can be restored on
 *       upgrade.
 *   - 'opportunistic' means that a durable value is persisted for post-upgrade
 *       restoration, but a non-durable value is still accepted (and will result
 *       in valueless restoration).
 *   - 'ignored' means that a value is not persisted for restoration even if it is
 *       durable.
 *
 ...
 * @property {boolean} hasValue hasValue indicates the presence of value. It
 *   starts off false, and can be reset to false when a durable publish kit is
 *   restored and the previous value was not durable, or non-terminal and
 *   valueDurablity is 'ignored'.
 * @property {any} value value holds either a non-terminal value from `publish`
 *   or a terminal value from `finish` or `fail`, depending upon the value in
 *   status.
 */
...
/**
 * @template T
 * @typedef {IterableEachTopic<T> & EachTopic<T> & SharableSubscription<T>} Subscription<T>
 ...
 *   getStoreKey: () => Promise<
 *     VStorageKey & { subscription: Subscription<T> }
 *   >;
