...
Example:
	ag-chain-cosmos testnet --v 4 --output-dir ./output --starting-ip-address 192.168.10.2
	`,
	...
	cmd.Flags().String(flagNodeDaemonHome, "ag-chain-cosmos",
		"Home directory of the node's daemon configuration")
	cmd.Flags().String(flagNodeCLIHome, "ag-cosmos-helper",
...
func InitTestnet(
	...
	agoricConfig := srvconfig.DefaultConfig()
	agoricConfig.MinGasPrices = minGasPrices
	...
	for i := 0; i < numValidators; i++ {
		...
		agoricConfigFilePath := filepath.Join(nodeDir, "config/server.toml")
		srvconfig.WriteConfigFile(agoricConfigFilePath, agoricConfig)
	}
