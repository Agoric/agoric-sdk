...
export default async function main(progname, args, { env, homedir, agcc }) {
  ...
  async function launchAndInitializeSwingSet(bootMsg) {
     ...
     * case, we publish these entries in vstorage under a dedicated prefix. This
     * effectively shadows the "export data" of the swingStore so that processes
     * like state-sync can generate a verified "root of trust" to restore
     * SwingSet state.
     *
