...
import type { WalletBridgeMsg } from '../../../src/types.js';
import {
  makeAssetRegistry,
  publishDepositFacet,
} from '../../../src/walletFactory.js';

export const prepare: typeof import('../../../src/walletFactory.js').prepare =
  async (zcf, privateArgs, baggage) => {
    ...
    const walletsByAddress: MapStore<string, SmartWallet> =
      provideDurableMapStore(baggage, 'walletsByAddress');
    ...
    const handleWalletAction = makeExo(
      ...
      {
        fromBridge: async (obj: WalletBridgeMsg) => {
      ...
      }),
      {
        provideSmartWallet(
          ...
          namesByAddressAdmin: ERef<
            import('@agoric/vats/src/types.js').NameAdmin
          >,
        ): Promise<[SmartWallet, boolean]> {
          let makerCalled = false;
          const maker = async (): Promise<SmartWallet> => {
