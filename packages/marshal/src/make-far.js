...
/**
 * The tag record that remotables inherit from.
 *
 * @param {Object} oldProto
 * The object that the new tag record should inherit from. This will typically
 * be the object that the `remotable` argument inherited from before it was
 * rewired to inherit from this tag record. The oldProto should be something
 * with a `toString` method that can be inherited. In fact, it will always
 * be either `Object.prototype` or one of `*Function.prototype`.
 * @param {InterfaceSpec} iface
 * @returns {Object}
 */
const makeRemotableTagRecord = (oldProto, iface) =>
  harden(
    create(oldProto, {
 ...
 * @returns {Object}
 */
const getRemotableOldProto = remotable => {
 ...
 * @param {Object=} oldProto
 * @returns {(remotable?: object) => object}
 */
export const makeRemotableMaker = (
  iface = 'Remotable',
  props = undefined,
  ...
  assert(iface);
  // TODO: When iface is richer than just string, we need to get the
  // allegedName in a different way.
  assert(props === undefined, X`Remotable props not yet implemented ${props}`);

  const remotableProto = makeRemotableTagRecord(oldProto, iface);
  ...
  return (remotable = {}) => {
    ...
    assert.equal(oldProto, getRemotableOldProto(remotable));
...
export const Remotable = (
  ...
  const oldProto = getRemotableOldProto(remotable);
  const makeRemotable = makeRemotableMaker(iface, props, oldProto);
  return makeRemotable(remotable);
};
harden(Remotable);

/**
 * A concise convenience for the most common `makeRemotableMaker` use.
 *
 * @template T
 * @param {string} farName
 * @param {Object=} oldProto
 * @returns {(remotable?: T) => T}
 */
export const makeFarMaker = (farName, oldProto = Object.prototype) => {
  return makeRemotableMaker(`Alleged: ${farName}`, undefined, oldProto);
};
harden(makeFarMaker);
 ...
 */
export const Far = (farName, remotable = {}) => {
  const oldProto = getRemotableOldProto(remotable);
  const makeFar = makeFarMaker(farName, oldProto);
  return makeFar(remotable);
};
harden(Far);
