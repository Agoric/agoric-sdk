...
https://github.com/Agoric/agoric-sdk/issues Please add a [store]
prefix to the title and `store` tag to store-related issues.
...
* run `yarn test` within `packages/store` and make sure all the unit
...
* `git tag -a store-v$VERSION -m "store-v$VERSION"`
* `yarn publish --access public`
* `git push`
* `git push origin store-v$VERSION`
