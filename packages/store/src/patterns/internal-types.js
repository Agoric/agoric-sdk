 ...
 * @property {KeyKind=} keyKindName
 * @property {PatternKind=} patternKindName
 *
 * @property {(candidate: Passable) => boolean} doIsKey
 * Only called if `passStyleOf(candidate) === passStyleName`
 * Returning true means `keyKindOf(candidate) === keyKindName`
 *
 * @property {(candidate: Passable) => boolean} doIsPattern
 * Only called if `passStyleOf(candidate) === passStyleName`
 * Returning true means `patternKindOf(candidate) === patternKindName`
 *
