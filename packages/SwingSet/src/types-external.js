 ...
 * @property {string} [bundleCachePath] if present, SwingSet will use a bundle
 *   cache at this path
 ...
 */
/**
 * @typedef {KernelOptions & SwingSetOptions} SwingSetConfig a swingset config
 *   object
 */
 ...
 * @property {boolean} [restartWorkerOnSnapshot] Reload worker immediately upon
 *   snapshot creation
 */
 ...
 *
 * These two sources use StaticVatOptions and DynamicVatOptions respectively
 * (the dynamic options are more restrictive, but can include a Meter object).
 * The dynamic `createVat()` process creates a run-queue event named
 * 'create-vat', which carries a form named InternalDynamicVatOptions (which can
 * include a MeterID integer).
 *
 * For both types, when we finally create the vat, the options are converted
 * into RecordedVatOptions, which is plain data and gets stored in the DB
 ...
 * @property {any} [vatParameters]
 * @property {boolean} [enableSetup] If true, permits the vat to construct
 *   itself using the `setup()` API, which bypasses the imposition of LiveSlots
 *   but requires the vat implementation to enforce the vat invariants manually.
 *   If false, the vat will be constructed using the `buildRootObject()` API,
 *   which uses LiveSlots to enforce the vat invariants automatically. Defaults
 *   to false.
 * @property {boolean} [enablePipelining] If true, permits the kernel to
 *   pipeline messages to promises for which the vat is the decider directly to
 *   the vat without waiting for the promises to be resolved. If false, such
 *   messages will be queued inside the kernel. Defaults to false.
 * @property {boolean} [useTranscript] If true, saves a transcript of a vat's
 *   inbound deliveries and outbound syscalls so that the vat's internal state
 *   can be reconstructed via replay. If false, no such record is kept. Defaults
 *   to true.
 * @property {number | 'never'} [reapInterval] The interval (measured in number
 *   of deliveries to the vat) after which the kernel will deliver the
 *   'bringOutYourDead' directive to the vat. If the value is 'never',
 *   'bringOutYourDead' will never be delivered and the vat will be responsible
 *   for internally managing (in a deterministic manner) any visible effects of
 *   garbage collection. Defaults to the kernel's configured
 *   'defaultReapInterval' value.
 ...
 *   OptManagerType &
 *   OptEnableDisavow &
 *   OptNodeOptions} StaticVatOptions
 *
 ...
 *   that can be used to upgrade it with new code or parameters, terminate it,
 *   or be notified when it terminates.
 * @property {() => Promise<any>} done returns a promise that will be fulfilled
 ...
 * ) => Promise<VatUpgradeResults>} upgrade
 ...
 * ) => import('@endo/far').ERef<CreateVatResults>} createVat
 */
