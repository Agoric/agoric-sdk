  ...
  const slogDuration = async (labels, startProps, fn, ...args) => {
    ...
    const finish = extraProps => {
      ...
      if (finished) {
        ...
        const msg = `slog event ${startLabel} already finished; ignoring props ${q(extraProps || {})}`;
        const err = Error(msg);
        console.error(err);
      ...
      if (extraProps) {
        ...
        if (hasOwn(extraProps, 'type') || hasOwn(extraProps, 'seconds')) {
          ...
          const { type: _, seconds: __, ...validProps } = extraProps;
    ...
    try {
      ...
      if (!finished) {
        console.error(Error(`unfinished slog event ${startLabel}`, { cause }));
      }
    }
  };
