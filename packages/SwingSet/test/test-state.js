...
test('storage helpers', t => {
 ...
 t.assert(s.has('foo.0'));
 t.falsy(s.has('foo.1'));
 t.falsy(s.has('foo.2'));
 t.falsy(s.has('foo.3'));
 t.falsy(s.has('foo.4'));
 t.assert(s.has('foo.5'));
  checkState(t, () => getAllState(storage), [
    ['foo.0', 'f0'],
    ['foo.5', 'f5'],
  ]);
...
test('kernelKeeper promises', async t => {
 ...
 t.assert(k.hasKernelPromise(p1));
 ...
 t.assert(k2.hasKernelPromise(p1));
