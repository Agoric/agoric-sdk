...
test('failed upgrade - lost promise export', async t => {
  ...
  const assertSpyCalls = async (message, newCalls) => {
    ...
    // Where we expect a promise and there is actually a promise in the right
    // position, update our expectation to that exact instance.
    const expectation = [];
    for (let i = 0; i < expectSpyCalls.length; i += 1) {
      const expectCall = expectSpyCalls[i];
      const [expectMethodName, ...expectArgs] = expectCall;
      const [actualMethodName, ...actualArgs] = actualSpyCalls[i] || [];
      if (actualMethodName !== expectMethodName) {
    ...
    arrayIsLike(t, actualSpyCalls, expectation, message);
  };
