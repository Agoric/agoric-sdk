...
/**
 * @callback AssertMakeError The `assert.error` method, recording details for the console.
 *
...
/**
 * @callback AssertFail The `assert.fail` method.
 *
 *   Fail an assertion, recording details to the console and raising an exception
 *   with just type information.
 *
 ...
 *   augmented console, like the causal console of `console.js`, to provide
 *   extra information associated with logged errors.
 * @param {Error} error
 ...
 * This states that the argument should appear quoted (as if with
 * `JSON.stringify`), in the error message of the thrown error. The payload
 * itself is still passed unquoted to the console as it would be without `quote`.
 *
 ...
 * ```
 *
 * The normal convention is to locally rename `quote` to `q` and `details` to `X`
 *
 * ```js
 ...
 *
 * ```js
 * assert.equal(sky.color, color, X`${sky.color} should be ${q(color)}`);
 * ```
 *
...
/**
 * @callback Raise To make an `assert` which terminates some larger unit of
 *   computation like a transaction, vat, or process, call `makeAssert` with a
 *   `Raise` callback, where that callback actually performs that larger
 *   termination. If possible, the callback should also report its `reason`
 *   parameter as the alleged reason for the termination.
 * @param {Error} reason
 */

/**
 * @callback MakeAssert Makes and returns an `assert` function object that
 *   shares the bookkeeping state defined by this module with other `assert`
 *   function objects made by `makeAssert`. This state is per-module-instance
 *   and is exposed by the `loggedErrorHandler` above. We refer to `assert` as a
 ...
 *   error messages. The assertion functions take such messages as optional arguments:
 *
 *   ```js
 ...
 *   itself with the formatted message in two ways. It will report the real
 *   details to the console but include only the typeof information in the
 *   thrown error to prevent revealing secrets up the exceptional path. In the
 *   example above, the thrown error may reveal only that `sky.color` is a
 *   string, whereas the same diagnostic printed to the console reveals that the
 *   sky was green.
 *
 *   The `raw` member of a `template` is ignored, so a simple `string[]` can also
