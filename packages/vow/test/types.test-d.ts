...
// ===== watch() unwraps its result with Fulfilled<> =====
//
// `vt.watch(specimen, watcher?, ...args)` resolves to `Vow<Fulfilled<R>>`,
// where `Fulfilled<>` recursively peels Vow/Promise layers off the result.
// Before the `Fulfilled<>` wrapper was added to watch's return type, watching
// a vow-of-vow (or a watcher returning a vow-of-vow) produced a `Vow<Vow<...>>`
// that callers had to unwrap twice. These cases pin the single-Vow invariant.

// Case 1: no watcher — a nested-vow specimen collapses to a single Vow.
{
  const nested = null as unknown as Vow<Vow<{ a: 'b' }>>;
  expectType<Vow<{ a: 'b' }>>(vt.watch(nested));
}

// Case 2: a watcher whose onFulfilled returns Vow<Vow<T>> still yields
// Vow<T>, not Vow<Vow<T>>.
{
  const specimen = null as unknown as Vow<number>;
  const watcher = {
    onFulfilled: (_n: number) => null as unknown as Vow<Vow<string>>,
  };
  expectType<Vow<string>>(vt.watch(specimen, watcher));
}

// Case 3: a plain (non-vow) watcher result is wrapped in exactly one Vow.
{
  const specimen = null as unknown as Vow<number>;
  const watcher = { onFulfilled: (n: number) => n + 1 };
  expectType<Vow<number>>(vt.watch(specimen, watcher));
}
