      ...
      reporter: {
        upsertPendingTx(txId: TxId, txMeta: TxMeta, status: TxStatus) {
          ...
          const value: PublishedTx = {
            destinationAddress: txMeta.destinationAddress,
            type: txMeta.type,
            ...(txsWithAmounts.includes(txMeta.type)
              ? { amount: txMeta.amountValue }
              : {}),
            ...(txMeta.type === TxType.MAKE_ACCOUNT
              ? { expectedAddr: txMeta.expectedAddr }
              : {}),
            status,
          };
