...
export const buildModel = (
  ...
  const ensureConstraint = (name: string) => (constraints[name] ??= {});
  const objectiveTerm: Record<string, number> = {};
  for (const edge of graph.edges) {
    ...
    const outC = ensureConstraint(`node_${edge.src}`);
    outC[flowVar] = (outC[flowVar] || 0) + 1;
    const inC = ensureConstraint(`node_${edge.dest}`);
  ...
  if (Object.keys(ints).length) model.ints = ints;
  return model;
};
...
const { steps } = await planRebalanceFlow({
  assetRefs: ['Aave_Arbitrum', 'Compound_Arbitrum', 'USDN', '<Deposit>', '@agoric'],
  current: {
