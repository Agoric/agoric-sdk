...
```text
portfolioValueMicroUsd * instrumentPortion * 10_000
  <= instrumentTvlUsd * 1_000_000 * totalPortions * maxVaultShareBps
```
...
check a delegated request before changing policy or creating a flow.

Benefits:

- The planner cannot choose either the TVL or the portfolio-value estimate.
- Rejection is synchronous and occurs before policy mutation.
...
type PlanObservations = {
  ...
  instrumentTvls: Partial<
    Record<InstrumentId, { tvlUsd: bigint; asOf: number }>
  >;
};
```
...
- `asOf` supports audit but does not prove provenance or freshness. This design
  trusts the planner's collection policy unless explicit age/skew checks are
  added.
