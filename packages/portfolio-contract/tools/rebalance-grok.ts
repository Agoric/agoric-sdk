...
export type RebalanceScenario = {
  ...
  after: Record<YieldProtocol, Dollars>;
...
> => {
  const result: Partial<Record<YieldProtocol, Dollars>> = {};
  if (aave && !['no portfolio', 'TODO'].includes(aave))
    result.Aave = aave as Dollars;
  if (compound && !['TODO'].includes(compound))
    result.Compound = compound as Dollars;
  if (usdn) result.USDN = usdn as Dollars;
  return result;
};
  ...
  for (const row of data) {
    ...
    if (label === 'Description') {
      ...
      currentScenario.after = {
        Aave: (aave || '$0.00') as Dollars,
        Compound: (compound || '$0.00') as Dollars,
        USDN: (usdn || '$0.00') as Dollars,
      };
