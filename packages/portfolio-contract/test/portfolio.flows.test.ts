...
test('swap rejects unconfigured token values', async t => {
  ...
  await t.throwsAsync(
    ...
    { message: /not configured as a source token/ },
  );
  await t.throwsAsync(
    () =>
      swapRewardToStable(baseCtx, gInfo, amount, {
    ...
    { message: /not configured as a destination token/ },
  );
  await t.throwsAsync(
    () =>
      swapRewardToStable(baseCtx, gInfo, amount, {
        ...baseSwap,
        tokenIn: '0x0000000000000000000000000000000000000abc',
        tokenOut: '0x0000000000000000000000000000000000000abc',
      }),
    { message: /cannot be the same as tokenOut/ },
  );

  const extraSwapToToken = harden({
    caipChainId: 'eip155:43114',
    chainName: 'Avalanche',
    tokenId: '0x0000000000000000000000000000000000000fed',
    symbol: 'USDT',
    decimals: 6,
    usage: ['swapTo'],
  });
  const multiSwapToCtx = {
    ...baseCtx,
    addresses: {
      ...contractsMock.Avalanche,
      usdc: '0x0000000000000000000000000000000000000bad',
    },
    chainMetadata: harden({
      ...chainMetadataMock,
      Avalanche: {
        ...chainMetadataMock.Avalanche,
        tokenMetadataById: {
          ...chainMetadataMock.Avalanche.tokenMetadataById,
          [makeTokenIdKey(extraSwapToToken.tokenId)]: extraSwapToToken,
        },
      },
    }),
  } as unknown as EVMContext;
  await t.throwsAsync(
    () =>
      swapRewardToStable(multiSwapToCtx, gInfo, amount, {
        ...baseSwap,
        tokenIn: '0x0000000000000000000000000000000000000abc',
      }),
    {
      message:
        /tokenOut must be specified when "Avalanche" has 2 configured destination tokens/,
    },
  );
});
...
test('provideNobleAccount reserves once under concurrency', async t => {
  ...
  t.is(
    ...
    'both callers must observe the same Noble ICA',
  );
  ...
  t.is(
    ...
    1,
    'two callers must not attempt multiple NFA registrations',
  );
});
...
test('provideNobleAccount registers the ICA and NFA before resolving', async t => {
  ...
  t.false(
    ...
    'there must be no Noble account before provisioning',
  );
  ...
  t.truthy(nfaTransfer, 'NFA registration transfer must be sent');
  const memo = JSON.parse((nfaTransfer as any).opts.memo);
  t.truthy(memo.noble.forwarding, 'transfer must carry the forwarding memo');

  // The reservation resolved only after both the ICA and the NFA were in place.
  t.true(
    'noble' in kit.reader.accountIdByChain(),
    'Noble account must be resolved after ICA + NFA',
  );
});
...
test('provideNobleAccount releases on makeAccount failure and a retry re-attempts', async t => {
  ...
  t.false(
    'noble' in kit.reader.accountIdByChain(),
    'reservation must be released after the failure',
  );
  ...
  t.true(
    'noble' in kit.reader.accountIdByChain(),
    'retry must provision the Noble account',
  );
});
...
test('provideNobleAccount releases on NFA registration failure and a retry re-attempts', async t => {
  ...
  t.truthy(firstRegistration, 'the first ICA registration must be attempted');
  t.false(
    'noble' in kit.reader.accountIdByChain(),
    'reservation must be released after the registration failure',
  );
  ...
  t.is(
    ...
    'retry must register the existing ICA instead of leaking it and creating another',
  );
});
...
test('pure EVM deposit does not provision an unused Noble account', async t => {
  ...
  const plannerP = (async () => {
    ...
    t.true(
      'agoric' in kit.reader.accountIdByChain(),
      'Agoric LCA must be present',
    );
    t.false(
      'noble' in kit.reader.accountIdByChain(),
      'Noble account must not be present immediately after open',
    );
    t.is(
  ...
  t.false(
    'noble' in kit.reader.accountIdByChain(),
    'a +Arbitrum to @Arbitrum flow must not create a Noble account',
  );
});
...
test('openPortfolio replay path keeps eager Noble provisioning for EVM', async t => {
  ...
  t.true(
    'noble' in kit.reader.accountIdByChain(),
    'config-less legacy invocation must preserve eager Noble provisioning',
  );
  t.deepEqual(
    reserveCalls,
    ['agoric', 'noble'],
...
test('openPortfolio from EVM: Noble registration failure fails the flow, not the open', async t => {
  ...
  t.false(
    'noble' in kit.reader.accountIdByChain(),
    'Noble reservation must be released after registration failure',
  );
