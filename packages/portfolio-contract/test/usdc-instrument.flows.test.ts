...
import { openPortfolio as rawOpenPortfolio } from '../src/portfolio.flows.ts';
...
test('openPortfolio and deposit to USDC instrument / cash', async t => {
  ...
  const createAndDepositP = rawOpenPortfolio(
    orch,
    ctx,
    seat,
  ...
  const gmpRaw = offer.log.find(
    e =>
      e._method === 'transfer' &&
      (e as { address?: { chainId?: string } }).address?.chainId ===
        'axelar-dojo-1',
  );
  ...
  t.snapshot(
    {
      methods: offer.log.map(({ _method }) => _method),
      statusAfterOpen,
      gmpDecoded,
    },
