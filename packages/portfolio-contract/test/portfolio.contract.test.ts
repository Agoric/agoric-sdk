...
testLegacyAndRouter(
  test.macro({
    ...
    async exec(t, useRouter: boolean) {
      const { common, planner1, txResolver, evmTrader } = await setupPlanner(
        t,
        { useRouter },
      );
  ...
  test.macro({
    ...
    async exec(t, useRouter: boolean) {
      const { common, planner1, started, readPublished, txResolver } =
        await setupPlanner(t, { useRouter });
      const { usdc, bld } = common.brands;
      ...
      t.log(
        `predicted ${evm} depositFactory address`,
      ...
      const flowHistory =
        contents[`${storagePath}.flows.flow${depositFlowNum}`];
      t.truthy(
        Array.isArray(flowHistory) &&
          flowHistory.some(entry => entry?.state === 'done'),
...
testLegacyAndRouter(
  test.macro({
    ...
    async exec(t, useRouter: boolean) {
      ...
      t.truthy(
        statusBefore.accountIdByChain?.[openChain],
