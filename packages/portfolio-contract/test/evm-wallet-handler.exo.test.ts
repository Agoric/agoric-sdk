...
type AutoFeaturesVariant = {
  operation: 'OpenPortfolio' | 'OpenPortfolioWithAutoFeatures';
  namePrefix: string;
};

const openPortfolioAutoFeaturesMacro = test.macro({
  title: (_providedTitle, { operation }: AutoFeaturesVariant) =>
    `handleOperation - openPortfolio with auto-features (${
      operation === 'OpenPortfolio'
        ? 'optional features field'
        : 'legacy OpenPortfolioWithAutoFeatures type'
    })`,
  exec: async (t, { operation, namePrefix }: AutoFeaturesVariant) => {
    const { zone } = t.context;
    const {
      vowTools,
      getCalls,
    ...
    // Create an OpenPortfolio operation with auto-features enabled (the
    // optional `features` field), or the dedicated (pre-optional-fields)
    // OpenPortfolioWithAutoFeatures operation, which remains supported for
    // backward compatibility.
    const openPortfolioOperationDetails: YmaxOperationDetails<
      typeof operation
    > &
      Required<Pick<FullMessageDetails, 'permitDetails'>> = {
      operation,
    ...
    } as YmaxOperationDetails<typeof operation> &
      Required<Pick<FullMessageDetails, 'permitDetails'>>;
...
test(openPortfolioAutoFeaturesMacro, {
  operation: 'OpenPortfolio',
  namePrefix: 'test1b_',
});
test(openPortfolioAutoFeaturesMacro, {
...
type GrantVariant = {
  operation: 'OpenPortfolio' | 'OpenPortfolioWithGrant';
  namePrefix: string;
};

const openPortfolioGrantMacro = test.macro({
  title: (_providedTitle, { operation }: GrantVariant) =>
    `handleOperation - openPortfolio with grant (${
      operation === 'OpenPortfolio'
        ? 'optional grantee field'
        : 'legacy OpenPortfolioWithGrant type'
    })`,
  exec: async (t, { operation, namePrefix }: GrantVariant) => {
    const { zone } = t.context;
    const {
      vowTools,
      getCalls,
    ...
    // Create an OpenPortfolio operation with a grantee (the optional
    // `grantee` field), or the dedicated (pre-optional-fields)
    // OpenPortfolioWithGrant operation, which remains supported for backward
    // compatibility: create the portfolio AND delegate control to an
    // automation agent in a single signed message.
    ...
    t.is(
      ...
      1,
      `${operation} routes to openPortfolioFromEVM`,
    );
