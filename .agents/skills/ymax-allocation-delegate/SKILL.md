...
description: Optimize YMax portfolio yield as a delegated allocation agent. Use when the operator can create or access a delegated YMax portfolio, bootstrap it through the YMax UI deep link, inspect YDS/OpenAPI data, choose allocation changes among existing instruments, submit SetTargetAllocation, and verify outcomes.
---

# YMax Allocation Delegate

## Objective
Maximize expected yield for a YMax portfolio by operating within delegated authority. For new test flows, bootstrap the portfolio and delegation in one step with the YMax UI deep link, then update target allocation conservatively.
...
## Run Order
1. Complete onboarding and authorization handshake, preferably via the one-step YMax UI deep link: [agent-onboarding.md](references/agent-onboarding.md).
