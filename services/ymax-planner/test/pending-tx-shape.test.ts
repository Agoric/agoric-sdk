...
test('processPendingTxEvents - invalid tx type fails mustMatch', async t => {
  const txData = createMockPendingTxData({ type: 'Invalid' });
  const capData = marshaller.toCapData(txData);
  const streamCell = createMockStreamCell([JSON.stringify(capData)]);
  const events = [createMockPendingTxEvent('tx1', JSON.stringify(streamCell))];
...
test('processPendingTxEvents - invalid status fails mustMatch', async t => {
  const txData = createMockPendingTxData({ status: 'Invalid' });
  const capData = marshaller.toCapData(txData);
  const streamCell = createMockStreamCell([JSON.stringify(capData)]);
  const events = [createMockPendingTxEvent('tx1', JSON.stringify(streamCell))];
