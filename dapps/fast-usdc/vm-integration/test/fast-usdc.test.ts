...
import {
  ...
  insistManagerType,
  makeSwingsetHarness,
} from '@agoric/boot/tools/supports.js';
  ...
  async t => {
    ...
    if (freshDeploy) {
      ...
      const materials = buildProposal(
        nodeRequire.resolve('../scripts/start-fast-usdc.build.js'),
        ['--net', 'MAINNET'],
      );
      await evalProposal(materials);
    }
...
test.serial('upgrade; update noble ICA', async t => {
  ...
  const materials = await buildProposal(
    nodeRequire.resolve('../scripts/fast-usdc-reconfigure.build.js'),
  );
  await evalProposal(materials);
...
test.serial('distributes fees per BLD staker decision', async t => {
  ...
  for (const { dest, args, rxd } of cases) {
    ...
    const materials = buildProposal(
      nodeRequire.resolve('../scripts/fast-usdc-fees.build.js'),
...
test.serial('replace operators', async t => {
  ...
  const addOperators = buildProposal(
    nodeRequire.resolve('../scripts/add-operators.build.js'),
