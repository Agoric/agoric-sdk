...
1. List every created promise and every async task that can become detached from its consumer (`void`, un-awaited `.then(...)`, fire-and-forget helper, or a promise saved and only observed after a later interleaving point).
2. For each detached or delayed-observation task, answer: "What keeps this from running past flow Done?"
