...
while IFS= read -r file; do
  if [ -f "$file" ]; then
    ...
    has_push=$(grep -c "\.push(" "$file" 2>/dev/null)
    has_pop=$(grep -c "\.pop(" "$file" 2>/dev/null)
    has_splice=$(grep -c "\.splice(" "$file" 2>/dev/null)
    has_shift=$(grep -c "\.shift(" "$file" 2>/dev/null)
    has_unshift=$(grep -c "\.unshift(" "$file" 2>/dev/null)
    has_sort=$(grep -c "\.sort(" "$file" 2>/dev/null)
    has_reverse=$(grep -c "\.reverse(" "$file" 2>/dev/null)
    
    total=$(expr $has_push + $has_pop + $has_splice + $has_shift + $has_unshift + $has_sort + $has_reverse)
...
echo -e "\nTotal files with potential mutations: $(grep -c '^FILE:' /tmp/copyarray-analysis/26-readonly-mutation-analysis.txt)"
