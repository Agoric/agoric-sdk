...
async function buildSwingset(mailboxStateFile, kernelStateFile, withSES, vatsDir, argv, broadcast) {
...
console.log(`MAILBOX: ${mb}`);
  ...
  console.log(`deliverInboundMbx calling Kernel `);
      await processKernel();
    } else {
  console.log(`deliverInboundMbx ELSE `);
    }
  }
...
export default async function start(basedir, withSES, argv) {
  ...
  const d = await buildSwingset(mailboxStateFile, kernelStateFile, withSES, vatsDir, argv, broadcast);
