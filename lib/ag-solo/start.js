...
async function buildSwingset(stateFile, withSES, vatsDir) {
  ...
  return { deliverInbound, deliverInboundCommand,
...
export default async function start(basedir, withSES) {
  ...
  connections.forEach(async c => {
    if (c.type === 'chain-cosmos-sdk') {
      ...
      // chain nodes
      const deliverator = await connectToChain(basedir, c.GCI, c.rpcAddresses, c.myAddr, inbound, c.chainID);
      ...
      broadcastJSON = makeHTTPListener(basedir, c.port, command);
    } else {
  ...
  const d = await buildSwingset(stateFile, withSES, vatsDir);
