...
export default async function start(basedir, withSES, argv) {
  ...
  const contracts = (await fs.promises.readdir(contractsDir)).reduce(
    (prior, name) => {
      const match = name.match(/^(\w+)-([^.]+)/);
      if (match && prior[match[1]]) {
        prior[match[1]].push(`${match[2]}=${contractsDir}/${name}`);
      }
      return prior;
    },
    { zoe: [], contract: [] },
  ...
  for (const [kind, pairs] of Object.entries(contracts)) {
    if (pairs.length > 0) {
      const target = kind === 'contract' ? 'contractHost' : 'zoe';
      // eslint-disable-next-line no-await-in-loop
      await uploadContract(basedir, [`--target`, target, ...pairs]);
    }
  }
}
