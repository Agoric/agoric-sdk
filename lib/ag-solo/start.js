...
async function buildSwingset(stateFile, withSES, vatsDir, argv) {
  ...
  async function pollTimerDevice(intervalMillis) {
    async function poll() {
      if (timer.poll(intervalMillis)) {
        await processKernel();
      }
    }
    console.log(`setting up TIMER: ${timer}. will call poll()`);
    setInterval(poll, intervalMillis);
  }
  // TODO(hibbert) Choose something non-arbitrary
  const blockPeriod = 5000;
  pollTimerDevice(blockPeriod);
