...
function build(E, D) {
  ...
  return {
    ...
    setPresences(ps) {
      ...
      if (homeObjects.canvasStatePublisher) {
        const subscriber = harden({
          notify(m) {
            // m will be a string, JSON.stringify([[],[],..])
            // created by ERTP/more/pixels/gallery.js setPixelState()
            D(commandDevice).sendBroadcast({ type: 'updateCanvas', state: m });
          },
        });
        console.log(`subscribing to canvasStatePublisher`);
        // TODO: this should provoke an immediate update
        E(homeObjects.canvasStatePublisher).subscribe(subscriber);
      }
    },
