...
function build(E) {
  ...
  async function startup(mint, http) {
    ...
    sharedDustMint = await E(mint).makeMint('dust');
    sharedDustIssuer = await E(sharedDustMint).getIssuer();
  ...
  async function getChainBundle(nickname) {
    ...
    const purse = await E(sharedDustIssuer).makeEmptyPurse();
    const dustIssuer = sharedDustIssuer;
    const chainBundle = { gallery, handoffService, purse, dustIssuer};
