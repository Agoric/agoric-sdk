  ...
  return helpers.makeLiveSlots(
    ...
    (E, D) => {
      async function createPrivateBundle(vats, contractHost, zoe, registrar) {
        ...
        const wallet = fillTwoPurses(vats.mints, vats.wallet, 'Moola purse', 'Simolean purse');
        ...
        return { uploads, spawner, /* http: httpRegistrationCallback */ wallet };
      }
      ...
      // Make services that are provided on the chain side, real or virtual
      function makeChainBundler(vats, timerDevice) {
        return harden({
          async createDemoBundle(nickname, jackpot) {
            ...
            const chainTimerService = await addTimerService(timerDevice, vats.timer);
          ...
          if (ROLES.chain || ROLES.one_chain) {
            ...
            const demoProvider = harden({
              ...
              async getDemoBundle(nickname) {
                return chainBundler.createDemoBundle(nickname);
              },
            });
            await Promise.all(
              [bootAddress, ...additionalAddresses].map(addr =>
            ...
            const bundle = await E(demoProvider).getDemoBundle();
            const privateBundle = createPrivateBundle(
                vats, bundle.contractHost, bundle.zoe, bundle.registrar);
            await E(vats.http).setPresences(
              { ...bundle, localTimerService },
              privateBundle,
            );
            ...
            await setupCommandDevice(vats.http, devices.command, {client: true});
            const bundle = await makeChainBundler(vats, devices.timer)
              .createDemoBundle('localuser');

            // Setup of the Local part /////////////////////////////////////
            const privateBundle =
                await createPrivateBundle(vats, bundle.contractHost, bundle.zoe, bundle.registrar);
            E(vats.http).setPresences(bundle, privateBundle);
