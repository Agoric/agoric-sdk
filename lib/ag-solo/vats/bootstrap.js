...
export default function setup(syscall, state, helpers) {
  ...
  console.log(`SOLO created DEMO bundle`);
            return harden({
              ...pixelBundle,
              timerHandler,
              handoff,
    ...
    console.log(`PROVISIONING:   SC2 ${nickname}`);
          ...
          if (ROLES.two_client) {
            ...
            await E(vats.http).setCommandDevice(devices.command, {client: true});
      console.log(`SCENARIO#2: local client setCommandDevice done`);
            D(devices.command).registerInboundHandler(vats.http);
            await addRemote(GCI);
            const demoProvider = await E(vats.comms).addIngress(GCI, SC2_INDEX);
      console.log(`SCENARIO#2: local client demoProvider set`);
            const bundle = await E(demoProvider).getDemoBundle();
            await E(vats.http).setPresences(bundle, await createPrivateBundle(vats));
      console.log(`SCENARIO#2: local client bundle available`);
