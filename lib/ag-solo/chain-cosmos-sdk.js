...
export async function connectToChain(
  ...
  const getClient = () =>
    retryRpcAddr(async rpcAddr => {
      ...
      const { validators } = await rpc.validators({ height: 1 });
      rpc.close();
      if (genesis.chain_id !== chainID) {
	throw Error(`downloaded chainID ${genesis.chain_id} does not match expected chainID ${chainID}`);
      }
      const gci = createHash('sha256').update(djson.stringify(genesis)).digest('hex');
      if (gci !== GCI) {
      ...
      const clientState = {
        validators,
	commit: null,
	header: { height: 1, chain_id: chainID },
      };
