...
export async function connectToChain(
  ...
  async function queuedHelper(name, maxQueued, args, parseReturn, stdin) {
    ...
    try {
      return await retryHelper(args, parseReturn, stdin, async () => {
  ...
  */
  ...
  c.lightClient.on('update', _a => {
    ...
    return getMailbox()
      .then(({ outbox, ack }) => inbound(GCI, outbox, ack))
