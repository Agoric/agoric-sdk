...
export async function connectToChain(basedir, GCI, rpcAddresses, myAddr, inbound) {
  ...
  // We spawn a long-running copy of 'sscli', the swingset-flavored
  // cosmos-sdk CLI tool. We tell that process to listen on its REST port,
  // and then we make HTTP calls to that port. This is the middle ground
  // between our original hackish approach (shelling out to a brand new copy
  // of sscli each time we wanted to send a message) and the
  // too-hard-to-do-right-this-instant less-hackish approach (building an FFI
  // binding to the same golang code that sscli uses, and calling it
  // directly).

  // the 'sscli' tool in our repo is built by 'make install' and put into the
  // user's $GOPATH/bin . That's a bit intrusive, ideally it would live in
  // the build tree along with bin/css-solo . But for now we assume that
  // 'sscli' is on $PATH somewhere.
  ...
  const serverDir = path.join(basedir, 'sscli-statedir');
  ...
  const p = spawn('sscli', args);
  p.stdout.on('data', _data => 'ignored');
  //p.stdout.on('data', data => console.log(`sscli rest-server stdout: ${data}`));
  p.stderr.on('data', data => console.log(`sscli rest-server stderr: ${data}`));
  p.on('close', code => console.log(`sscli rest-server exited with ${code}`));
  console.log(`started sscli rest-server`);

  const mailboxURL = `http://localhost:1317/swingset/mailbox/${myAddr}`;
  ...
  */
  ...
  // the genesis block, which we can compare against the GCI). The 'sscli'
  // tool might do it. We need one subscription-type thing to tell us that a
  // new block exists, then we can use a different query-type tool to
  // retrieve the outbox, but we want to know that the outbox is correctly
  // traced to the block header, and that the block header is a legitimate
  // descendant of our previously-validated state.
...
/*
    const url = `${rpcURL}/swingset/mailbox`;
    const body = {
      base_req: {
        from: myAddr,
        password: 'mmmmmmmm',
