...
function run() {
  ...
  const commands = [];
  function addEntry(histnum, command, _display) {
  ...
  // Create an illegal color fill.
  const pcanvas = document.createElement('canvas');
  const checkerSize = PIXEL_SIZE / 8;
  pcanvas.width = checkerSize * 2;
  pcanvas.height = pcanvas.width;
  const pctx = pcanvas.getContext('2d');
  pctx.fillStyle = '#cccccc';
  for (let i = 0; i < 2; i += 1) {
    for (let j = 0; j < 2; j += 1) {
      pctx.fillStyle = (i + j) % 2 === 0 ? '#ffffff' : '#cccccc';
      pctx.fillRect(i * checkerSize, j * checkerSize, checkerSize, checkerSize);
    }
  }
  ...
  function updateCanvas(state) {
    ...
    function renderPixel(x, y, color) {
      ...
      // Default to checkered pattern if not a valid color.
      ctx.fillStyle = pattern;
      // This assignment does nothing if there was not a valid fillStyle.
      ctx.fillStyle = color;
      ctx.fill();
      ctx.closePath();
    }
