...
export default function setGCIIngress(basedir, GCI, rpcAddresses) {
  const myAddrFn = path.join(basedir, 'sscli-address');
  ...
  fs.writeFileSync(fn, JSON.stringify(connections)+'\n');

  let gciFileContents = `export const GCI = '${GCI}';\n`;
