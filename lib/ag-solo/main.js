...
export default async function solo(progname, rawArgv) {
  ...
  const {_: argv, ...opts} = parseArgs(rawArgv, {boolean: ['controller', 'help', 'version'], default: {
    webport: '8000',
    webhost: '127.0.0.1',
  }});
...
`)
  ...
  if (argv[0] === 'init') {
    ...
    const isController = opts.controller;
    const rpcAddresses = argv.slice(2);
    setGCIIngress(basedir, GCI, rpcAddresses, chainID, isController);
  }
