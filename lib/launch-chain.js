...
export async function launch() {
  ...
  async function deliverInbound(sender, messages, ack) {
    ...
    const oldMailboxData = JSON.stringify(mbs.exportToData());
    if (mb.deliverInbound(sender, messages, ack)) {
      await controller.run();
      // now check mbs
      const newMailboxData = JSON.stringify(mbs.exportToData());
      if (newMailboxData !== oldMailboxData) {
        console.log(`new outbound messages!`);
        set("mailbox", newMailboxData);
      }
    }
  }
