...
export function launch() {
  const withSES = false;
  const { controller, inbound, mb, mbs } = buildSwingset(withSES);

  async function deliverInbound(sender, messages, ack) {
    if (!messages instanceof Array) {
      throw new Error(`inbound given non-Array: ${messages}`);
    }
    const oldMB = JSON.stringify(mb.exportToData());
    if (mb.deliverInbound(sender, messages, ack)) {
      await controller.run();
      // now check mbs
      if (JSON.stringify(mb.exportToData()) !== oldMB) {
  ...
  async function handler(action) {
    ...
    if (action.type === 'DELIVER_INBOUND') {
      return deliverInbound(action.peer, action.messages, action.ack);
    }
