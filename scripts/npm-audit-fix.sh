git clone https://github.com/AgoricBot/ERTP.git
cd ERTP
git remote add upstream https://github.com/Agoric/ERTP.git
git remote set-url origin https://AgoricBot:$GITHUB_TOKEN@github.com/AgoricBot/ERTP.git
git fetch upstream
git checkout master
...
if [ "$files_changed" = true ] ; then
  ...
  hub pull-request --no-edit --base Agoric/ERTP:master
fi
