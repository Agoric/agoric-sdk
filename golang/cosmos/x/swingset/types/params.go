...
func validateMsgTypeBeansPerUnit(i interface{}) error {
	...
	for _, entry := range v {
		...
		if err := validateBeansPerUnit(entry.Beans); err != nil {
			return fmt.Errorf("msg type %s beans must be valid: %s", entry.MsgTypeUrl, err)
		}
	}
	return nil
}
...
func validateBeanFeeBurnFraction(i interface{}) error {
	...
	for _, coin := range v {
		if err := sdk.ValidateDenom(coin.Denom); err != nil {
			return fmt.Errorf("bean fee burn fraction denom %s must be valid: %s", coin.Denom, err)
		}
...
func validateMinGasPrice(i interface{}) error {
	...
	for _, coin := range v {
		if err := sdk.ValidateDenom(coin.Denom); err != nil {
			return fmt.Errorf("minimum gas price denom %s must be valid: %s", coin.Denom, err)
		}
		if coin.Amount.IsNegative() {
