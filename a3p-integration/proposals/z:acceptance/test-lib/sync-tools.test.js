...
import { waitUntilAccountFunded, waitUntilContractDeplyed, waitUntilInvitationReceived, waitUntilOfferResult } from './sync-tools.js';

const sampleOfferStatus = {
    "status": {
        "id": 1726389716785,
        "invitationSpec": {
            "invitationMakerName": "CloseVault",
            "previousOffer": "1711355582081",
            "source": "continuing"
        },
        "numWantsSatisfied": 1,
        "payouts": {
            "Collateral": {
                "brand": "[Alleged: SEVERED: stTIA brand {}]",
                "value": "10347167"
            },
            "Minted": {
...
const sampleBalanceUpdate = {
    "currentAmount": {
        "brand": "[Alleged: SEVERED: Zoe Invitation brand {}]",
        "value": []
    },
    "updated": "balance"
}
...
const sampleInstance = [
    [
        "ATOM-USD price feed",
        "[Alleged: SEVERED: InstanceHandle {}]"
    ],
    [
        "Crabble",
        "[Alleged: SEVERED: InstanceHandle {}]"
    ...
    [
        "scaledPriceAuthority-stOSMO",
        "[Alleged: SEVERED: InstanceHandle {}]"
    ],
    [
        "scaledPriceAuthority-stTIA",
        "[Alleged: SEVERED: InstanceHandle {}]"
    ],
    [
        "scaledPriceAuthority-stkATOM",
        "[Alleged: SEVERED: InstanceHandle {}]"
    ],
    [
    ...
    [
        "stOSMO-USD price feed",
        "[Alleged: SEVERED: InstanceHandle {}]"
    ],
    [
        "stTIA-USD price feed",
        "[Alleged: SEVERED: InstanceHandle {}]"
    ],
    [
        "stkATOM-USD price feed",
        "[Alleged: SEVERED: InstanceHandle {}]"
    ],
    [
        "walletFactory",
...
test.serial('wait until contract is deployed', async t => {
    ...
    const waitP = waitUntilContractDeplyed('name', {
        follow,
        setTimeout: globalThis.setTimeout
    }, { maxRetries: 5, retryIntervalMs: 1000, log: t.log, errorMessage: "Contact not deplyed yet" });
...
test.serial('wait until account funded', async t => {
    ...
    const waitP = waitUntilAccountFunded(
        ...
        { maxRetries: 5, retryIntervalMs: 1000, log: t.log, errorMessage: "Account not funded yet" }
    )
...
test.serial('wait until offer result, balance update - should throw', async t => {
    const { setValue, follow } = makeFakeFollow();
...
test.serial('wait until offer result, numWantsSatisfied not equals to 1 - should throw', async t => {
    const { setValue, follow } = makeFakeFollow();
    setValue({ status: { id: 'my-offer', numWantsSatisfied: 0 }, updated: "offerStatus" });
...
test.serial('wait until offer result, do not wait for "payouts"', async t => {
    ...
    setTimeout(() => setValue({ status: { id: 'my-offer', numWantsSatisfied: 1 }, updated: "offerStatus" }), 1000); // First, offer is seated
    setTimeout(() => setValue({ status: { id: 'my-offer', numWantsSatisfied: 1, result: 'thank you' }, updated: "offerStatus" }), 3000); // First, offer is resulted
...
test.serial('wait until offer result, wait for "payouts"', async t => {
    ...
    setTimeout(
        () => setValue({ status: { id: 'my-offer', numWantsSatisfied: 1 }, updated: "offerStatus" }),
        1000
    ); // First, offer is seated
    setTimeout(
        () => setValue({ status: { id: 'my-offer', numWantsSatisfied: 1, result: 'thank you' }, updated: "offerStatus" }),
        3000
    ); // Now, offer is resulted
    setTimeout(
        () => setValue({ status: { id: 'my-offer', numWantsSatisfied: 1, result: 'thank you', payouts: {} }, updated: "offerStatus" }),
        4000
...
test.serial('wait until invitation recevied, wrong "updated" value', async t => {
    const { setValue, follow } = makeFakeFollow();
    setValue({ updated: "offerStatus" });
...
test.serial('wait until invitation recevied, falty "currentAmount" object', async t => {
    const { setValue, follow } = makeFakeFollow();
    ...
    setTimeout(() => setValue({ updated: "balance", currentAmount: { foo: true } }), 2000);

    await t.throwsAsync(waitP);
});

test.serial('wait until invitation recevied, brand string do not match', async t => {
    const { setValue, follow } = makeFakeFollow();
...
test.only('wait until invitation recevied', async t => {
    const { setValue, follow } = makeFakeFollow();
    setValue({});
    ...
    setTimeout(() => setValue({ updated: 'balance', currentAmount: { brand: '[Alleged: SEVERED: Zoe Invitation brand {}]'}}), 2000);
