...
const assetInfo = {
  repo: {
    name: 'wallet-factory1',
    description: 'first upgrade of wallet factory vat',
  },
...
await step('bundles not yet installed', async () => {
  ...
  for (const { bundles, evals } of info) {
    ...
    for (const bundle of bundles) {
      ...
      assert(!loaded.includes(id), `missing ${id}`);
    }
  }
});
...
await step('ensure enough IST to install bundles', async () => {
  ...
  await ensureISTForInstall(agd, config, 2 * totalSize, {
    log: console.log,
  });
});
