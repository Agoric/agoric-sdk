...
const staticConfig = {
  ...
  buildAddAssetInfo: Object.values({
    ...assetInfo.buildAssets.addOracle,
    ...assetInfo.buildAssets.addAsset,
  }),
...
test.serial('make sure oracle addresses include gov1 and gov2', async t => {
  const { agd } = t.context;
  const starsOraclesRW = makeFileRW(
    'testAssets/addStarsAsset/add-STARS-oracles.js',
    { fsp, path },
  );
...
test.serial('make sure user1 will be funded with STARS', async t => {
  const { agd } = t.context;
  const fundStarsRW = makeFileRW('testAssets/fundStars/fund-stars-prop.js', {
    fsp,
    path,
  });
...
test.serial('add STARS asset', async t => {
  const propDir =
    '/usr/src/proposals/b:liquidation-visibility/testAssets/addStarsAsset';
  const bundleInfos = await readBundles(propDir);
  await passCoreEvalProposal(bundleInfos, {
    title: `Core eval of ${propDir}`,
    installer: 'user1',
  });
  t.log(bundleInfos);
  t.pass();
...
test.serial('fund user1 with STARS', async t => {
  const propDir =
    '/usr/src/proposals/b:liquidation-visibility/testAssets/fundStars';
  const bundleInfos = await readBundles(propDir);
  await passCoreEvalProposal(bundleInfos, {
    title: `Core eval of ${propDir}`,
    installer: 'user1',
...
test.serial('add STARS collateral', async t => {
  const propDir =
    '/usr/src/proposals/b:liquidation-visibility/testAssets/addStarsCollateral';
  const bundleInfos = await readBundles(propDir);
  await passCoreEvalProposal(bundleInfos, {
    title: `Core eval of ${propDir}`,
    installer: 'user1',
